...
// swap them over an installed h0x-ADE and macOS treats each build as a new app.
...
// Electron's loader requirements alongside h0x-ADE's headless-host dependencies.
...
module.exports = {
  appId,
  productName: 'h0x-ADE',
  protocols: [{ name: 'h0x-ADE', schemes: ['pavii-h0x'] }],
  ...
  files: [
    ...
    // Why: the Windows CLI shim ships via extraResources to resources/bin/h0x.cmd
    // (beside the native resources/bin/h0x.exe). Packing the source tree into
    // app.asar too lets asarUnpack:['resources/**'] extract a second copy at
    // app.asar.unpacked/resources/win32/bin/h0x.cmd with no adjacent h0x.exe,
  ...
  nsis: {
    artifactName: 'h0x-windows-setup.${ext}',
  ...
  mac: {
    // Why rank Alternate: h0x-ADE joins Finder's "Open With" list for Markdown without claiming
    ...
    extendInfo: {
      NSAppleEventsUsageDescription:
        'h0x-ADE allows terminal-launched developer tools to automate local apps when you request it.',
      NSBluetoothAlwaysUsageDescription:
        'h0x-ADE allows terminal-launched developer tools to access Bluetooth devices when you request it.',
      NSBluetoothPeripheralUsageDescription:
        'h0x-ADE allows terminal-launched developer tools to access Bluetooth devices when you request it.',
      NSCameraUsageDescription: "Application requests access to the device's camera.",
      NSLocationUsageDescription:
        'h0x-ADE allows terminal-launched developer tools to access location when you request it.',
      NSLocalNetworkUsageDescription:
        'h0x-ADE allows terminal-launched developer tools to discover and connect to local development servers when you request it.',
      NSMicrophoneUsageDescription: "Application requests access to the device's microphone.",
      NSAudioCaptureUsageDescription:
        'h0x-ADE allows terminal-launched developer tools to capture desktop audio when you request it.',
  ...
  dmg: {
    artifactName: 'h0x-macos-${arch}.${ext}'
  },
  linux: {
    // Why mimeTypes and not fileAssociations: shared-mime-info already maps *.md/*.markdown to
    // text/markdown, so reusing that type puts h0x-ADE in the Open With list without shipping a glob
    ...
    desktop: {
      entry: {
        ...
        // GNOME docks need an exact match to group it with h0x.desktop.
        StartupWMClass: 'h0x'
      }
    },
    extraResources: [
  ...
  appImage: {
    artifactName: isLinuxArm64Release ? 'h0x-linux-arm64.${ext}' : 'h0x-linux.${ext}'
  },
  deb: {
    packageName: 'h0x',
    artifactName: 'h0x_${version}_${arch}.${ext}',
    // Why: xvfb lets the bundled `h0x serve` CLI run browser panes on a headless
    ...
    // Why: symlink the bundled CLI onto PATH at install time so `h0x serve`
  ...
  // on Intel Macs. The beforeBuild hook performs h0x-ADE's targeted rebuild and
  // returns false so electron-builder does not rebuild optional cpu-features.
  npmRebuild: true,
  publish: {
    provider: 'github',
    owner: 'vjk7989',
    repo: devChannelRepo ?? 'h0x-ade',
...
async function signMacComputerUseHelper(helperAppPath, packager) {
  if (!existsSync(helperAppPath)) {
    if (isMacRelease) {
      throw new Error(`Missing h0x-ADE Computer Use helper app at ${helperAppPath}`)
    }
    return
  }
  ...
  const identity =
    ...
    (isMacRelease ? null : '-')
  if (!identity) {
    throw new Error('Missing signing identity for h0x-ADE Computer Use helper app')
  }
  // Why: TCC grants attach to this nested app's code identity. Sign it before
  // the outer h0x-ADE.app is sealed so production builds preserve that identity.
