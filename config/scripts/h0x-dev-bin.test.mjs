...
const wrapperPath = path.join(projectDir, 'config', 'scripts', 'h0x-dev.mjs')

describe('h0x-dev package bin', () => {
  it('uses a Node entrypoint for cross-platform package installs', () => {
    expect(packageJson.bin['h0x-dev']).toBe('./config/scripts/h0x-dev.mjs')
    expect(readFileSync(wrapperPath, 'utf8')).toMatch(/^#!\/usr\/bin\/env node\n/)
  })

  it('runs the dev CLI through Node without requiring Bash', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'h0x-dev-bin-'))
