...
export const JA_UNIFIED_VALUES = {
  ...
  'Show h0x-ADE Mobile Button': 'h0x-ADE Mobile ボタンを表示',
  Paste: '貼り付け',
  ...
  'Show the h0x-ADE Mobile button at the top of the left sidebar.':
    '左サイドバー上部に h0x-ADE Mobile ボタンを表示します。',
  'Enable h0x-ADE CLI': 'h0x-ADE CLI を有効化',
  'Registered the h0x-ADE CLI in PATH.': 'h0x-ADE CLI を PATH に登録しました。',
  ...
  'Gitea credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart h0x-ADE if environment variables changed.':
    'Gitea の認証情報は設定されていますが、認証できませんでした。トークン、API ベース URL、リポジトリの権限を確認し、環境変数を変更した場合は h0x-ADE を再起動してください。',
  'Azure DevOps credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart h0x-ADE if environment variables changed.':
    'Azure DevOps の認証情報は設定されていますが、認証できませんでした。トークン、API ベース URL、リポジトリの権限を確認し、環境変数を変更した場合は h0x-ADE を再起動してください。',
  'Bitbucket credentials are configured but could not authenticate. Check the token and repository permissions, then restart h0x-ADE if environment variables changed.':
    'Bitbucket の認証情報は設定されていますが、認証できませんでした。トークンとリポジトリの権限を確認し、環境変数を変更した場合は h0x-ADE を再起動してください。',
  ...
  'h0x-ADE will continue to work, but commands such as h0x status, h0x terminal, and orchestration are unavailable for this session.\n\n{{guidance}}\n\nCause: {{cause}}':
    'h0x-ADE は引き続き使用できますが、このセッションでは h0x status や h0x terminal などのコマンドとオーケストレーションを利用できません。\n\n{{guidance}}\n\n原因: {{cause}}'
}
