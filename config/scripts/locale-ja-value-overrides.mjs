...
const JA_AUDIT_OVERRIDES = {
  ...
  'Split Up': '上に分割',
  'Split Down': '下に分割',
  "Install the Browser Use skill so agents can operate h0x-ADE's browser.":
    'Browser Use スキルをインストールすると、 Agent が h0x-ADE のブラウザを操作できるようになります。',
  'bold ·': '太字 ·',
  ...
  '{{host}} is no longer a saved SSH host, so this workspace is no longer connected to a live host. It can only be removed from h0x-ADE — files and branches on the remote are left untouched.':
    '{{host}} は保存済みの SSH ホストではなくなったため、このワークスペースは稼働中のホストに接続されていません。h0x-ADE から削除することのみ可能です。リモート上のファイルとブランチはそのまま残ります。',
  'The app shell could not finish rendering. Retry to remount it, or relaunch h0x-ADE if the error persists.':
    'アプリシェルのレンダリングを完了できませんでした。再マウントを試すか、エラーが解決しない場合は h0x-ADE を再起動してください。',
  'Workspaces are unavailable on a mobile-scope pairing. Reconnect using the browser access link from Settings → Runtime Environments → Share this h0x-ADE server.':
    'モバイル向けペアリングではワークスペースを使用できません。[設定] → [リモート h0x-ADE サーバー] → [この h0x-ADE サーバーを共有する] から、ブラウザ用リンクで再接続してください。',
  'Change this later in Settings → Browser.': '後で [設定] → [ブラウザ] から変更できます。',
  'h0x-ADE Cloud sign-in is not configured': 'h0x-ADE Cloud へのサインインが設定されていません',
  ...
  'only when h0x-ADE cannot derive the API base URL from the git remote.':
    'は、h0x-ADE が git リモートから API のベース URL を取得できない場合のみ設定します。',
  'only when h0x-ADE cannot derive the API URL from the remote.':
    'は、h0x-ADE がリモートから API URL を取得できない場合のみ設定します。',
  Set: '設定:',
  '. Set': '。設定:',
  ', or set': '、または',
  and: 'と',
  ...
  ' vs {{value0}}': ' vs {{value0}}',
  'When you create a workspace, h0x-ADE refreshes the remote base and safely fast-forwards your matching local branch, such as':
    'ワークスペースを作成すると、h0x-ADE はリモートベースを更新し、一致するローカルブランチを安全に fast-forward します。対象は',
  '. This keeps commands like': '。これにより、',
  'from comparing against stale history. h0x-ADE skips the update if that branch has uncommitted changes or local-only commits.':
    'などのコマンドが古い履歴と比較されるのを防ぎます。対象のブランチにコミットしていない変更やローカルのみのコミットがある場合、h0x-ADE は更新をスキップします。',
  ...
  'Jump to...': '移動先…',
  'Start work': '作業を開始',
  'Remove {{value0}} contained {{value1}} from h0x-ADE':
    '含まれている {{value1}} {{value0}} 件を h0x-ADE から削除',
  'People in {{orgName}} who can collaborate on h0x-ADE.':
    '{{orgName}} で h0x-ADE を共同利用できるメンバーです。',
  'People in your organization who can collaborate on h0x-ADE.':
    '組織内で h0x-ADE を共同利用できるメンバーです。',
  ...
  'This QR code grants limited (mobile) access. To use the full web app, open the browser access link from Settings → Runtime Environments → Share this h0x-ADE server → New Link.':
    'この QR コードでは、モバイル向けの制限付きアクセスのみ利用できます。完全版の Web アプリを使用するには、[設定] → [リモート h0x-ADE サーバー] → [この h0x-ADE サーバーを共有する] → [新規リンク] から、ブラウザ用アクセスリンクを開いてください。',
  'Loading skills...': 'スキルを読み込み中…',
  'Start agents on your tasks without leaving h0x-ADE':
    'h0x-ADE から離れずに、タスクから Agent を開始',
  'Workspace created from {{value0}}, but h0x-ADE could not fast-forward local {{value1}} because {{value2}}':
    'ワークスペースは {{value0}} から作成されましたが、{{value2}} のため h0x-ADE はローカル {{value1}} を fast-forward できませんでした',
  ...
  'Rename h0x-ADE-created branches from the initial agent task.':
    'h0x-ADE が作成したブランチの名前を、最初の Agent タスクに基づいて変更します。',
  'Dim files matched by .gitignore in the file explorer.':
    'ファイルエクスプローラーで、.gitignore に一致するファイルを薄く表示します。',
  'Command line h0x-ADE runs when a text recipe uses Custom command.':
    'テキストレシピで「カスタムコマンド」を使用したときに h0x-ADE が実行するコマンドライン。',
  ...
  'Repository not in h0x-ADE': 'このリポジトリは h0x-ADE に登録されていません',
  ...
  'Clears them from h0x-ADE only. Remote files, worktrees, and branches are left untouched.':
    'h0x-ADE からのみ削除します。リモートのファイル、ワークツリー、ブランチはそのまま残ります。',
  ...
  'Install the h0x-ADE CLI before running agent skill setup.':
    'Agent スキルのセットアップを実行する前に、h0x-ADE CLI をインストールしてください。',
  'Run grok in a terminal on the computer running h0x-ADE and wait for it to start. If prompted, complete sign-in, then retry usage. You do not need to send a chat message.':
    'h0x-ADE を実行しているコンピュータのターミナルで grok を実行し、起動するまで待ってください。サインインを求められた場合は完了してから、使用状況を再取得してください。チャットメッセージを送る必要はありません。',
  ...
  "h0x-ADE works with every CLI agent. Choose the one you'll reach for most. Switch any time.":
    'h0x-ADE はすべての CLI Agent で動作します。最もよく使うものを選択してください。いつでも切り替えられます。',
  ...
  'Install and manage experimental h0x-ADE plugins.':
    '実験的機能の h0x-ADE プラグインをインストール・管理します。',
  "Resident memory held by h0x-ADE plus the processes under each worktree's terminals.":
    'h0x-ADE が保持する常駐メモリと、各ワークツリーのターミナル配下で実行中のプロセス。',
  ...
  'Optional. h0x-ADE can use your normal Codex login; add accounts only if you want quick switching in h0x-ADE.':
    '任意。h0x-ADE は通常の Codex ログインを使用できます。h0x-ADE 上ですばやく切り替えたい場合にのみ、アカウントを追加してください。',
  'Optional. h0x-ADE can use your normal Claude login; add accounts only if you want quick switching without moving chat sessions.':
    '任意。h0x-ADE は通常の Claude ログインを使用できます。チャットセッションを移さずにすばやく切り替えたい場合にのみ、アカウントを追加してください。',
  'Optional. h0x-ADE works with your existing provider logins; add accounts only if you want h0x-ADE to help switch between them.':
    '任意。h0x-ADE は既存のプロバイダーのログインと連携します。h0x-ADE に切り替えを任せたい場合にのみ、アカウントを追加してください。',
