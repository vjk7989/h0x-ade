  ...
  it('fixes Korean round 5 review, integration, and search keyword regressions', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Install the h0x-ADE skill so agents know to use the h0x-ADE CLI.',
        localeValue: '에이전트가 h0x-ADE CLI 사용 방법을 알 수 있도록 h0x-ADE 기술을 설치합니다.',
        locale: 'ko'
      })
    ).toBe('에이전트가 h0x-ADE CLI를 사용하도록 h0x-ADE 스킬을 설치하세요.')
    expect(
      repairTranslatedValue({
