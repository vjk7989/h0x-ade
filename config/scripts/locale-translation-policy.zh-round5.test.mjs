...
describe('locale-translation-policy zh round 5', () => {
  it('fixes brand spacing, hosted review, and h0x-ADE Mobile regressions', () => {
    expect(
      repairTranslatedValue({
        key: 'auto.components.stats.ShareUsageCard.0eb31e79ee',
        enValue: 'h0x-ADE IDE',
        localeValue: 'h0x-ADE集成开发环境',
        locale: 'zh'
      })
    ).toBe('h0x-ADE IDE')
    expect(
      repairTranslatedValue({
        ...
        enValue: 'h0x-ADE first',
        localeValue: 'h0x-ADE第一',
        locale: 'zh'
      })
    ).toBe('h0x-ADE 优先')
    expect(
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Show h0x-ADE Mobile Button',
        localeValue: '显示 h0x-ADE 移动按钮',
        locale: 'zh'
      })
    ).toBe('显示 h0x-ADE Mobile 按钮')
    expect(
      repairTranslatedValue({
  ...
  it('keeps brand names English and generic workflow terms translated', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue:
          'Git could not safely delete branch "{{value0}}"{{value1}}, so h0x-ADE kept it to avoid losing local commits.',
        localeValue:
          'Git 无法安全删除分支“{{value0}}”{{value1}}，因此 h0x-ADE 保留它以避免丢失本地提交。',
        locale: 'zh'
      })
    ).toBe('Git 无法安全删除分支“{{value0}}”{{value1}}，因此 h0x-ADE 保留它以避免丢失本地提交。')
  })
