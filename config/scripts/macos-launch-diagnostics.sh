#!/usr/bin/env bash
# Capture one-shot macOS launch diagnostics for a published h0x-ADE release.
#
# Usage:
...
write_environment_report() {
  {
    ...
    echo "$SHELL"
    echo
    echo "## current h0x-ADE processes before diagnostics"
    pgrep -fl 'h0x-ADE|Orca|orca' || true
  } >"$OUT_DIR/environment.txt"
}

ensure_no_existing_orca() {
  local existing
  existing="$(pgrep -x 'h0x-ADE|Orca' || true)"
  if [[ -z "$existing" ]]; then
    return 0
  fi

  {
    echo "An h0x-ADE process is already running. Close h0x-ADE and run this script again."
    echo
...
start_log_stream() {
  local file="$1"
  local predicate='process == "h0x-ADE" OR process == "Orca" OR eventMessage CONTAINS[c] "h0x-ADE" OR eventMessage CONTAINS[c] "Orca" OR eventMessage CONTAINS[c] "com.stablyai.orca"'
...
latest_orca_pid_for_app() {
  pgrep -nf "$APP_DIR/Contents/MacOS/h0x-ADE" || true
}
...
run_direct_exec_probe() {
  ...
  ELECTRON_ENABLE_LOGGING=1 \
    ...
    "$APP_DIR/Contents/MacOS/h0x-ADE" >"$stdout_file" 2>"$stderr_file" &
  local runner_pid="$!"
  wait_for_probe "$runner_pid" "$label"
  echo "ended_utc=$(date -u '+%Y-%m-%dT%H:%M:%SZ')" >>"$OUT_DIR/$label.meta"
  stop_log_stream "$stream_pid"
