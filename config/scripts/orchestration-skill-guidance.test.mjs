...
describe('orchestration skill guidance', () => {
  it('keeps external browser routing at the OS/page boundary', () => {
    ...
    expect(description).toContain(
      "Use Computer Use for external browser windows, webviews, h0x-ADE app UI, or desktop UI outside h0x-ADE's embedded browser only when the task requires OS/window-level control such as focus, menus, dialogs, coordinates, or screenshots."
    )
    expect(description).toContain(
  ...
  it('requires Orca runtime state before claiming a worker was orchestrated', () => {
    ...
    expect(toolBoundary).toContain('create the Task with `h0x orchestration task-create`')
    expect(toolBoundary).toContain('preferred `h0x orchestration worker-start` composition')
    expect(toolBoundary).toContain('low-level `h0x orchestration dispatch --inject` path')
    expect(toolBoundary).not.toContain('or `h0x orchestration run`')
    expect(skill).toContain(
      '`coordinator-start`, `coordinator-stop`, `run`, and `run-stop` are retired scheduler commands'
    )
    expect(toolBoundary).toContain(
      'Do not substitute non-h0x-ADE subagent tools, generic agent-spawn APIs, or chat-only parallel worker features'
    )
    expect(toolBoundary).toContain('do not create h0x-ADE task/dispatch provenance')
    ...
    expect(toolBoundary).toContain('h0x orchestration task-list --json')
    expect(toolBoundary).toContain('h0x orchestration dispatch-show --task <task_id> --json')
    expect(toolBoundary).toContain(
  ...
  it('keeps full handoffs out of dispatch lifecycle and off the active branch base', () => {
    ...
    expect(skill).toContain(
      '`--no-parent` only controls h0x-ADE lineage; it does not choose the Git base.'
    )
    expect(skill).toContain(
    ...
    expect(fullHandoffs).toContain(
      'Before creating a new worktree from an active feature branch, decide and state whether the desired h0x-ADE lineage is child or top-level'
    )
    expect(fullHandoffs).toContain(
  ...
  it('makes settled worker terminal release an explicit coordinator step', () => {
    ...
    expect(workerLoop).toContain(
      'h0x orchestration worker-start --task <next_task_id> --terminal <handle> --json` so h0x-ADE ' +
    ...
    expect(workerLoop).toContain('h0x orchestration worker-retain --dispatch <dispatch_id> --json')
    expect(workerLoop).toContain(
  ...
  it('keeps agent-first launch, handle recovery, and inbox injection distinct', () => {
    ...
    expect(agentFirstExample).not.toContain('h0x terminal list')
...
describe('orchestration install stub', () => {
  it('points at the version-matched guide and preserves the safe resolver', () => {
    ...
    expect(stub).toContain('H0X skills get orchestration')
  ...
  it('does not tell agents to mutate orchestration state before loading the guide', () => {
    ...
    expect(preGuide).not.toContain('h0x orchestration task-create')
    expect(preGuide).not.toContain('h0x orchestration dispatch')
  })
