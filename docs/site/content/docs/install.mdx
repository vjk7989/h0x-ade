...
<Callout title="Don't like the current update">
  ...
  page](https://github.com/vjk7989/h0x-ade/releases). Orca will not force-downgrade your worktree data
  if you go back.
</Callout>
...
The AppImage has a stable download link per architecture — [`h0x-linux.AppImage`](https://github.com/vjk7989/h0x-ade/releases/latest/download/h0x-linux.AppImage) for x64 and [`h0x-linux-arm64.AppImage`](https://github.com/vjk7989/h0x-ade/releases/latest/download/h0x-linux-arm64.AppImage) for arm64 — and needs `chmod +x` before its first run, because GitHub release assets carry no permission bits. The `.deb` and `.rpm` filenames carry the version and architecture, and the two formats spell architecture differently (`orca-ide_<version>_amd64.deb` or `_arm64.deb`; `orca-ide-<version>.x86_64.rpm` or `.aarch64.rpm`), so take those from the [Releases page](https://github.com/vjk7989/h0x-ade/releases) rather than a fixed URL.
