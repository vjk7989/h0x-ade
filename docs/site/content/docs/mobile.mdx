...
description: Pair the h0x-ADE mobile app to your desktop to monitor agents and unstick worktrees from your phone.
---

import { Callout } from '@/components/docs/prose'

The h0x-ADE mobile companion is an iOS/Android app that pairs with your desktop h0x-ADE and gives you a read-mostly view of running agents — agent status, recent terminal scrollback, and the controls you actually want from a phone (replying to a prompt, sleeping a worktree, reviewing source control, switching agent accounts). Pairing is one-time and the desktop is always the source of truth.
...
<Callout title="Beta">
  ...
  0.0.46](https://github.com/vjk7989/h0x-ade/releases/download/mobile-android-v0.0.46/app-release.apk).
</Callout>

## What you can do from mobile

- See every worktree, its agent, and its current status (working / done / waiting on input). Mobile lists worktrees from every host you're connected to (local desktops and remote h0x-ADE servers) in a single view, so switching between them doesn't need a device change.
...
1. On desktop, open h0x-ADE's pairing flow from the account / status menu. h0x-ADE shows a one-time pairing code.
1. On mobile, open the h0x-ADE companion app and choose **Pair**, then paste the code. (You can also follow a deep link from the desktop directly into the mobile pairing screen.)
1. Prefer **h0x Relay** for pairing when it is available — sign-in is required for Relay only. The local network address picker is optional on Relay (expand it when you want a LAN fast path); LAN pairing still needs an address.
...
For chat-capable agents (Claude, Codex, and others h0x-ADE recognizes), mobile can open the session in **Chat UI** instead of the raw terminal — the same idea as desktop's [Chat UI](/docs/agents/native-chat): a readable transcript, composer, permissions/questions, and image attachments.
...
- **Pairing fails** — make sure your desktop and phone are signed into the same h0x-ADE account. Pairing codes expire after a few minutes; generate a fresh one if it's been sitting on the screen.
...
- **Update h0x-ADE Mobile / Update h0x-ADE on your computer** — the phone and desktop speak a versioned mobile protocol. If either side is too old, mobile blocks that host and points you at the App Store (iOS), [GitHub Releases](https://github.com/vjk7989/h0x-ade/releases) (Android APK or desktop), then back to hosts. After updating, refresh the connection; if the message stays, remove the host and pair again.
...
<Callout title="Next steps">
  ...
  alerts reach the right device. For headless machines, use [Remote h0x-ADE
