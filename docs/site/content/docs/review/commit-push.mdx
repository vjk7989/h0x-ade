---
title: Commit & push from h0x-ADE
---

You can commit, push, and open the review without leaving h0x-ADE. The commit panel lives next to the diff viewer and is designed for the common case — review, stage, commit, push, move on.

## Commit

1. Stage changes by hunk or by file from the diff.
1. Write a commit message in the bottom panel, or use **Generate with AI** when you want h0x-ADE to draft one from the staged changes.
...
Pre-commit hooks from the repo run as usual. If a hook fails h0x-ADE surfaces the output inline.
...
**Push** pushes the worktree's branch to `origin`, setting upstream the first time. If the branch is behind, h0x-ADE will not silently force-push.
...
Amending is explicit — **Commit → Amend**. h0x-ADE won't amend commits that have already been pushed unless you confirm.
