...
- **h0x CLI** — register the bundled command-line tool for shells and agents.
...
- Auto-Rename Branch From Work — rename h0x-ADE-generated creature branches after an agent starts work.
...
- With remote or multi-host setups, commands are grouped by the h0x-ADE host that owns them (**Saved on**). Host ownership is separate from Global/Project scope and from where the command runs — see [Terminal → Quick Commands](/docs/terminal#quick-commands).
...
- **Agent status hooks** — show working / waiting / done states in h0x-ADE. Toggle takes effect without restarting the app (including the Windows WSL hook relay). CLI: `h0x agent hooks on|off|status`.
...
- **Link Routing** — open http(s) links from the terminal, markdown, and editor in h0x-ADE's browser or the system browser. Nested **Hold Shift…** inverts that default for one click (`⇧⌘-click` / `Shift+Ctrl+click`). See [Per-worktree browser](/docs/browser/overview#link-routing).
...
- **h0x-ADE account** — sign in (same account family as h0x Relay) to publish and manage shared files.
...
- Share from an open local HTML page or Markdown editor with **Share as artifact**, or via `h0x artifacts …`. See [CLI reference → Artifacts](/docs/cli/reference#artifacts).
...
- Bitbucket Cloud — **Connect** with **Email & API token** (default) or an **Access token**. h0x-ADE verifies the credential before saving. `ORCA_BITBUCKET_*` environment variables take precedence and hide Connect / Disconnect. Saved credentials stay on this machine — on a [Remote h0x-ADE Server](/docs/remote-servers), set the env vars on the server instead. See [Hosted reviews](/docs/review/github).
...
- **Microphone** — pick which input device dictation uses. Defaults to the system mic; choose a specific device (for example a headset) when you do not want the OS default. If the chosen mic is unplugged or missing, h0x-ADE falls back to the system default and shows a non-blocking notice.
...
- Pair and connect to remote h0x-ADE runtimes.
...
- **Delete workspace** defaults to `Cmd+Shift+Backspace` on macOS and `Ctrl+Shift+Backspace` on Windows / Linux. Hover the sidebar workspace you mean to delete; h0x-ADE still asks for confirmation.
...
- Plugin workers always run on this computer; SSH workspace actions still route through h0x-ADE.
...
- **Cloud VM** — show setup controls and workspace **Run on** targets for repo-owned, on-demand environments (cloud sandboxes, VMs, or local Docker). Setup guide and recipe install live under this experimental toggle. See [Ways to run h0x-ADE](/docs/ways-to-run#4-cloud-vms-per-workspace-environments).
