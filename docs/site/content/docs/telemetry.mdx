...
description: What anonymous usage data h0x-ADE collects, what it never collects, and how to opt out.
---

import { ImagePlaceholder } from '@/components/docs/prose'

This page describes the anonymous product-usage telemetry we collect in packaged h0x-ADE builds, what we never collect, and how to opt out.
...
Alongside each event we include basic build and platform information: the h0x-ADE version, your operating system (e.g. `darwin` / `win32` / `linux`), CPU architecture, coarse OS release string, release channel (`stable` or `rc`), and the anonymous local ID described above. No hostname, no username, no IP.
...
- **Agent errors** — a coarse error category and which agent kind was involved. We never see raw error messages or stack traces; per-incident detail stays in a local diagnostic trace file on your machine and only reaches h0x-ADE if you explicitly share a diagnostic bundle.
...
- No user account information (h0x-ADE has no account system).
...
**`ORCA_TELEMETRY_DISABLED=1`** — h0x-ADE-specific kill switch with the same semantics as
`DO_NOT_TRACK`.
...
- **Access:** project membership is restricted to a small number of h0x-ADE maintainers.
