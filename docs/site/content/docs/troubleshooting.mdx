...
- Open the terminal and run the agent's CLI manually. If it fails there, it's an auth or install problem in the CLI itself — not h0x-ADE.
- Make sure the CLI is on the `PATH` h0x-ADE sees (check [Settings → Agents](/docs/settings)).
- Try the **Restart** chip on the tab.

## Diff view looks wrong / stuck

- Click the refresh icon on the diff toolbar — h0x-ADE re-reads the worktree.
...
## h0x CLI says "command not found"

Register the CLI under [Settings → General → h0x CLI](/docs/settings). On macOS it installs a shim into `~/.local/bin`; make sure that's on your shell's `PATH`.
...
- Reconnect after installing tools so h0x-ADE can reinstall native modules.
...
- Use an SSH worktree (not a Remote h0x-ADE Server active runtime).
...
No tab is open in the current worktree. Open one with `h0x tab create --url ...` or open the browser pane manually and navigate.
...
**Help → Open Logs** opens the directory with h0x-ADE's logs. Attach these when filing a bug.
...
- [GitHub Issues](https://github.com/vjk7989/h0x-ade/issues) — bugs and feature requests.
