...
test('docs routes stay namespaced and the generated source uses /docs as its base URL', async () => {
  ...
  assert.match(ogImage, /public\/docs\/logo\.png/)
  assert.match(header, /src="\/docs\/logo\.png"/)
  assert.match(footer, /src="\/docs\/logo\.png"/)
  assert.ok(existsSync(path.join(publicRoot, 'docs', 'logo.png')))
  assert.ok(existsSync(path.join(publicRoot, 'docs', 'favicon.ico')))
  assert.equal(existsSync(path.join(publicRoot, 'logo.png')), false)
