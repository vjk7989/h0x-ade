...
export function diagnoseConnection(args: DiagnoseConnectionArgs): ConnectionDiagnosis {
  ...
  if (/relay director resolve failed \(503\)/i.test(evidence)) {
    ...
    return {
      likelyCause: `Relay service was temporarily unavailable${retryMs == null ? '.' : ` and asked h0x-ADE to retry in ${formatDelay(retryMs)}.`}`,
      nextStep: 'Keep h0x-ADE open; recovery should retry automatically.',
      reportability: 'none'
    }
  }
  ...
  if (/liveness-timeout|liveness timeout|connection health check failed/i.test(evidence)) {
    ...
    return {
      ...
      nextStep: 'h0x-ADE closed the stale session and started recovery.',
  ...
  if (/relay-session-failed|active relay session failed/i.test(evidence)) {
    return {
      ...
      nextStep: 'h0x-ADE started Relay recovery; the event history includes the cell close reason.',
      reportability:
    ...
    return {
      ...
      nextStep:
        args.pendingPath === 'relay'
          ? 'Relay recovery is in progress; keep h0x-ADE open while it retries.'
  ...
  if (/handshake-timeout|handshake timeout/i.test(evidence)) {
    return {
      likelyCause: 'The endpoint opened, but the encrypted h0x-ADE handshake did not finish.',
      nextStep: 'Confirm the desktop is running a compatible h0x-ADE version and retry.',
      reportability: 'none'
    }
  }
