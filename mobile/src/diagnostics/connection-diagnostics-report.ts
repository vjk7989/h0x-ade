...
export function buildConnectionDiagnosticsReport(args: {
  ...
  const lines: string[] = []
  lines.push('h0x-ADE Mobile connection diagnostics')
  lines.push(`Generated: ${new Date(now).toISOString()}`)
  lines.push(`App: h0x-ADE Mobile ${args.appVersion} · ${args.platform}`)
  const desktopAppVersion = normalizeHostAppVersion(args.desktopAppVersion)
  lines.push(`Host h0x-ADE version: ${desktopAppVersion ?? 'unknown'}`)
