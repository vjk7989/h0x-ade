    ...
    private func buildSnapshot(
        ...
        guard accessibilityTrustedSettled() else {
            ...
            throw ProviderError.coded(
                "permission_denied",
                "Accessibility permission is required for h0x-ADE Computer Use. Run `h0x computer permissions` or open Settings > Computer Use, grant Accessibility to h0x-ADE Computer Use, then retry."
            )
        }
        ...
        let screenshotStatus: ScreenshotStatus = if screenshot != nil {
            ...
            .failed("Screen Recording permission is required for h0x-ADE Computer Use; grant permission or pass --no-screenshot to inspect accessibility state only.")
...
private func focusedWindow(appElement: AXUIElement, app: AppDescriptor, visibleWindowCount: Int, allowRecovery: Bool) throws -> AXUIElement {
    ...
    if visibleWindowCount > 0 {
        ...
        throw ProviderError.coded("permission_denied", "app '\(app.name)' has visible windows but no accessibility window (AX reads stayed blocked for \(outcome.waitedMs)ms after retries). macOS Accessibility may need h0x-ADE Computer Use toggled off and on again in System Settings.")
    }
...
private final class PermissionWindowController: NSWindowController {
    ...
    convenience init(initialPermission: PermissionKind? = nil, terminateWhenDragAssistantCloses: Bool = false) {
        ...
        window.title = "Enable h0x-ADE Computer Use"
...
private enum PermissionKind: CaseIterable {
    ...
    var dragInstruction: String {
        switch self {
        case .accessibility:
            "Drag h0x-ADE Computer Use into the list above to allow Accessibility."
        case .screenshots:
            "Drag h0x-ADE Computer Use into the list above to allow Screenshots."
        }
    }
...
private final class PermissionView: NSView {
    ...
    private func build() {
        ...
        let titleText = checking
            ...
            : (ready ? "Computer Use is Ready" : "Enable h0x-ADE Computer Use")
        let title = label(titleText, size: 22, weight: .bold)
        let subtitle = label(
            checking
                ? "Checking Accessibility and Screenshots."
                : (ready
                    ? "h0x-ADE can use local apps when you ask."
                    : "Grant permissions so h0x-ADE can use apps when you ask."),
            size: 12,
            weight: .regular
        )
...
private final class PermissionDragAssistantController: NSWindowController {
    ...
    convenience init(
        ...
        window.title = "Drag h0x-ADE Computer Use"
...
private final class DraggableAppTile: NSView, NSDraggingSource {
    ...
    private func build() {
        ...
        let title = NSTextField(labelWithString: "h0x-ADE Computer Use")
...
private func isTrustedOrcaApplication(_ pid: pid_t) -> Bool {
    ...
    // h0x-ADE-owned bundle ids; the sidecar peer check must still authorize them.
...
private func runStdio() {
    fputs("h0x-ADE Computer Use provider must be launched by h0x-ADE in app-agent mode.\n", stderr)
    exit(13)
}
