...
Use this skill for desktop UI through `h0x computer`. For a website or web app, use it only when the page is in an external desktop browser window that needs desktop-level control. Do not use it for page-only automation: use `orca-cli` for h0x-ADE's embedded pages and a page-automation tool such as Playwright or CDP for external pages.
...
- Choose the h0x-ADE executable once: use the `ORCA_CLI_COMMAND` environment value when set;
  ...
  every platform.
- In every command example, `H0X` is a documentation placeholder — including examples that
  name a specific shell. Replace it with that chosen executable before running the command;
  do not create a shell variable or run `H0X` literally. Blocks that name no shell are
...
```text
H0X status --json
H0X computer capabilities --json
```

## Core Loop

```text
H0X computer list-apps --json
H0X computer get-app-state --app com.spotify.client --json
H0X computer click --app com.spotify.client --element-index 42 --json
```
...
```text
H0X computer get-app-state --app com.microsoft.edgemac --json
H0X computer get-app-state --app Spotify --json
H0X computer get-app-state --app pid:12345 --json
```
...
```text
H0X computer permissions --json
H0X computer capabilities --json
H0X computer list-apps --json
H0X computer list-windows --app <app> --json
H0X computer get-app-state --app <app> --json
H0X computer get-app-state --app <app> --restore-window --json
H0X computer click --app <app> --element-index <index> --json
H0X computer click --app <app> --x 100 --y 100 --json
H0X computer click --app <app> --x 100 --y 100 --modifiers CmdOrCtrl+Shift --json
H0X computer click --app <app> --element-index <index> --mouse-button right --json
H0X computer click --app <app> --element-index <index> --mouse-button middle --json
H0X computer perform-secondary-action --app <app> --element-index <index> --action <name> --json
H0X computer set-value --app <app> --element-index <index> --value "text" --json
H0X computer type-text --app <app> --text "text" --json
H0X computer press-key --app <app> --key Return --json
H0X computer hotkey --app <app> --key CmdOrCtrl+A --json
H0X computer paste-text --app <app> --text "text" --json
H0X computer scroll --app <app> (--element-index <index> | --x <x> --y <y>) --direction down --json
H0X computer drag --app <app> --from-element-index <index> --to-element-index <index> --json
H0X computer drag --app <app> --from-x 100 --from-y 100 --to-x 300 --to-y 300 --json
```
...
```bash
printf '%s' "$TEXT" | H0X computer set-value --app <app> --element-index <index> --value-stdin --json
```
...
```text
H0X computer get-app-state --app com.microsoft.edgemac --restore-window --json
H0X computer set-value --app com.microsoft.edgemac --element-index <addressBarIndex> --value "test123" --json
H0X computer press-key --app com.microsoft.edgemac --key Return --json
```
...
- `app_not_found`: run `list-apps` and retry with the bundle ID. If the target is a web app such as Gmail, choose the desktop browser app/window that contains it; do not retry `H0X computer ... --app Gmail` unchanged because `h0x computer` app selectors refer to desktop apps, not website names.
...
- `screenshot_failed`: use `--no-screenshot` if tree state is enough; if the message names Screen Recording or screenshots permission, run `H0X computer permissions --id screenshots --json`.
- `accessibility_error`: run `H0X computer capabilities --json`; if the message names Accessibility permission, run `H0X computer permissions --id accessibility --json`.
- Empty tree or no screenshot: app may have no visible window, be minimized, or need permissions.
- Permission errors: run `H0X computer permissions --json`, or `H0X computer permissions --id accessibility --json` / `--id screenshots --json` when the message names one permission, use the setup UI, then retry.

## Next Action

Confirm h0x-ADE status unless already checked, then run `H0X computer capabilities --json`. For external browser targets such as Gmail, identify the desktop browser app/window that contains the page, then get that target app state with `H0X computer get-app-state --app <app> --json`.
