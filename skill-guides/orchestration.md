...
name: orchestration
description: >-
  Use h0x-ADE orchestration for structured multi-agent coordination: threaded
  ...
  lightweight terminal prompts, shell commands, h0x-ADE worktree management,
  reading or waiting on terminals, and the h0x-ADE embedded browser. Use Computer
  Use for external browser windows, webviews, h0x-ADE app UI, or desktop UI
  outside h0x-ADE's embedded browser only when the task requires OS/window-level
  control such as focus, menus, dialogs, coordinates, or screenshots. Use
  `orca-cli` for h0x-ADE's embedded pages and a page-automation tool such as
...
```bash
h0x orchestration task-list --json
h0x orchestration dispatch-show --task <task_id> --json
```

If the work was accidentally run outside h0x-ADE orchestration, say so plainly. To repair provenance, rerun or revalidate the needed work through a fresh h0x-ADE terminal plus injected dispatch; do not retroactively describe the external worker as orchestrated.
...
- `h0x status --json` should show a running runtime.
- `orca` must be on PATH (`h0x` on Linux).
- The orchestration experimental feature must be enabled in Settings > Experimental.
- `h0x orchestration` commands are RPC calls to the running h0x-ADE runtime.
...
```bash
h0x orchestration run-list --json
# run_legacy_local is an empty audit tombstone after adoption.
h0x orchestration run-show --id run_legacy_local --json
# In run-list, find the ordinary Run whose objective is:
# "Recovered orchestration work from a contract update"
h0x orchestration run-show --id <adopted_run_id> --json
h0x orchestration task-list --run <adopted_run_id> --json
h0x orchestration inbox --full --json
h0x orchestration check --terminal <legacy_handle> --peek --format --json
h0x terminal read --terminal <legacy_handle> --json
h0x terminal wait --terminal <legacy_handle> --for tui-idle --timeout-ms 60000 --json
```
...
```bash
h0x orchestration run-use --id <adopted_run_id> --takeover-legacy --json
h0x orchestration check --run <adopted_run_id> --json
```
...
- Do not use `h0x orchestration dispatch --inject` for full handoffs. It injects a coordinator preamble that tells the worker to send `worker_done`, heartbeat, and `ask` messages, then end its turn under the original terminal's dispatch lifecycle.
- Do not run `h0x orchestration task-create`, `h0x orchestration dispatch --inject`, or `h0x orchestration check --wait` for full handoffs. Do not peek at terminal output after prompt delivery to monitor progress.
...
```bash
h0x orchestration send --subject <text> [--to <run:id|dispatch:id|legacy_handle>] [--from <handle>] [--body <text>] [--type <type>] [--priority <level>] [--thread-id <id>] [--payload <json>] [--json]
h0x orchestration check [--terminal <handle>] [--ack <delivery_id>] [--peek|--all] [--types <type,...>] [--format] [--wait] [--timeout-ms <n>] [--json]
h0x orchestration reply --id <msg_id> --body <text> [--from <handle>] [--json]
h0x orchestration ask (--question <text>|--resume <msg_id>) [--options <csv>] [--timeout-ms <n>] [--from <handle>] [--json]
h0x orchestration inbox [--limit <n>] [--json]
```
...
- While supervising workers manually, use `check --wait --types worker_done,escalation,question --timeout-ms <n>` instead of sleep/poll loops. Process the whole Delivery, reply to `question` messages with `h0x orchestration reply --id <msg_id> --body <answer> --json`, then acknowledge and keep waiting.
...
```bash
h0x orchestration run-create --objective <text> --json
h0x orchestration task-create --spec <text> [--deps <json_array>] [--parent <task_id>] [--json]
h0x orchestration task-list [--status <status>] [--ready] [--brief] [--json]
h0x orchestration task-update --id <task_id> --status <status> [--result <json>] [--json]
h0x orchestration dispatch --task <task_id> --to <handle> [--from <handle>] [--inject] [--json]
h0x orchestration dispatch-show --task <task_id> [--json]
```
...
- If the target is a bare shell, omit `--inject`, dispatch for tracking if needed, then send the prompt manually with `h0x terminal send --terminal <handle> --text <prompt> --enter --json`.
...
```bash
h0x orchestration run-create --objective "<objective>" --json
h0x orchestration task-create --spec "<worker A task>" --json
h0x orchestration task-create --spec "<worker B task>" --json
h0x orchestration worker-start --task <task_a> --worktree current --agent codex --json
h0x orchestration worker-start --task <task_b> --worktree current --agent claude --json
```
...
```bash
h0x orchestration worker-start --task <task_id> --worktree new-child --name <name> --agent codex --setup run --json
# Independent/top-level:
h0x orchestration worker-start --task <task_id> --worktree new-top-level --name <name> --agent codex --setup run --json
```
...
h0x orchestration worker-start --task <task_id> --on windows --worktree new-top-level --repo <exact_remote_repo_selector> --name <name> --agent codex --setup run --json
h0x orchestration worker-show --dispatch <dispatch_id> --json
h0x orchestration worker-read --dispatch <dispatch_id> --limit 50 --json
h0x orchestration send --to dispatch:<dispatch_id> --subject "Follow-up" --body "<attempt-specific guidance>" --json
```
...
```bash
h0x orchestration check --wait --types worker_done,escalation,question --timeout-ms 900000 --json
# Process every message. For each accepted worker_done that is not immediately reused:
h0x orchestration worker-release --dispatch <dispatch_id> --json
# Acknowledge only after every message and required release decision is handled:
h0x orchestration check --ack <delivery_id> --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```

After processing each accepted `worker_done`, choose the terminal's next owner before you acknowledge the Delivery or wait again. If the same exact agent has an immediate follow-up Task, read the `worker.agent_terminal_handle` field of `worker-show --dispatch <dispatch_id> --json`, then run `h0x orchestration worker-start --task <next_task_id> --terminal <handle> --json` so h0x-ADE transfers cleanup ownership to the new Dispatch. Otherwise run `h0x orchestration worker-release --dispatch <dispatch_id> --json`.

Run `worker-release` after both succeeded and failed `worker_done` reports unless the user explicitly asked to keep that worker live. Release is post-completion cleanup, not cancellation: h0x-ADE first preserves inspectable output, then closes only the exact agent terminal owned by that settled Dispatch. Reused or pre-existing terminals, setup terminals, coordinators, active workers, user-taken-over terminals, and identities h0x-ADE cannot prove are retained. If the user explicitly asks to keep the live terminal for debugging, record that exception with `h0x orchestration worker-retain --dispatch <dispatch_id> --json` instead of silently skipping cleanup. When the user is finished, the same Dispatch can be passed to `worker-release`, which clears the requested retention and releases the terminal.
...
```bash
h0x orchestration ask --question "<question>" --options "yes,no" --timeout-ms 600000 --json
h0x orchestration ask --resume <message_id> --timeout-ms 600000 --json
# Coordinator:
h0x orchestration reply --id <message_id> --body "<answer>" --json
```
...
```bash
h0x orchestration gate-create --task <task_id> --question <text> [--options <json_array>] [--json]
h0x orchestration gate-resolve --id <gate_id> --resolution <text> [--json]
h0x orchestration gate-list [--task <task_id>] [--status <status>] [--json]
```
...
Recovery only: `h0x orchestration reset --tasks|--messages|--all --json` clears the selected local orchestration database state. Do not run it during active coordination unless explicitly abandoning that state.
...
Do not run `h0x orchestration task-create`, `h0x orchestration dispatch --inject`, or `h0x orchestration check --wait` for full handoffs. `task-create` is also forbidden because it records coordinator-owned tracking state; if a task row is needed, the user asked for supervised orchestration. Do not create a `taskId`/`dispatchId`, inject a lifecycle preamble, wait for completion, or read the worker terminal after prompt delivery except to avoid losing the initial prompt.

New top-level worktree handoff:

```bash
h0x worktree create --name <task-name> --no-parent --agent codex --prompt "<task brief>" --setup run --json
```

Before creating a new worktree from an active feature branch, decide and state whether the desired h0x-ADE lineage is child or top-level. Use child worktree lineage only when the new work is conceptually stacked under or dependent on the active worktree. For independent repo-wide fixes, standalone feature work, or unrelated follow-up tasks, create a top-level worktree with `--no-parent`.
...
`h0x worktree create --agent codex --prompt ...` launches the known Codex agent but does not accept Codex-specific `--model` or `-c model_reasoning_effort=...` arguments. When the user asks for a specific Codex model or effort, create the independent worktree first, launch Codex with the requested command in that worktree, wait only for TUI readiness if prompt delivery would otherwise race startup, send the prompt, and stop.
...
Use the exact full `<repo-id>::<path>` worktree id returned by `h0x worktree create --json`; a bare repo id cannot target the new worktree.

```bash
h0x worktree create --name <task-name> --no-parent --setup run --json
h0x terminal create --worktree id:<newFullWorktreeId> --title <task-name> --command 'codex --model gpt-5.5 -c model_reasoning_effort="xhigh"' --json
...
`--no-parent` only controls h0x-ADE lineage; it does not choose the Git base. If the work should start from the repo default base, omit `--base-branch` so h0x-ADE uses that default, or explicitly pass the repo default base (`origin/main`, `origin/master`, or the `h0x repo show --repo <selector> --json` value); never base it on the current feature branch unless the user explicitly asks for stacked work or "branch from current". Put current-branch context in the prompt instead.
...
```bash
h0x terminal create --worktree active --title <task-name> --command "codex" --json
h0x terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
h0x orchestration dispatch --task <task_id> --to <handle> --inject --json
```
...
```bash
h0x worktree create --name <task-name> --agent codex --setup run --json
...
Use `h0x worktree create --prompt ...` or `h0x terminal send ...` for full handoffs or untracked/lightweight prompts. Those paths do not attach `taskId`/`dispatchId`; the worker should not send lifecycle messages unless the prompt supplies a live orchestration preamble.
...
```bash
h0x terminal list [--worktree <selector>] [--include-visual-layouts] [--json]
h0x terminal create [--worktree <selector>] [--title <text>] [--command <cmd>] [--json]
h0x terminal split --terminal <handle> [--direction horizontal|vertical] [--command <cmd>] [--json]
h0x terminal wait --terminal <handle> --for tui-idle --timeout-ms <n> --json
h0x terminal read --terminal <handle> --json
h0x terminal send --terminal <handle> --text <text> --enter --json
```

If an older CLI rejects `worktree create --agent`, create the worktree normally, then run `h0x terminal create --worktree <selector> --command "codex" --json` or `--command "claude"`.
...
```bash
h0x terminal create --worktree active --title login-css-worker --command "claude" --json
h0x terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
h0x orchestration task-create --spec "Fix the login button CSS" --json
h0x orchestration dispatch --task <task_id> --to <handle> --inject --json
h0x orchestration check --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```
