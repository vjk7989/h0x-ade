...
an `h0x vm recipe doctor` failure. h0x-ADE is a thin wrapper: you guide, detect, and scaffold;
...
```text
H0X skills get orca-per-workspace-env
```
...
```text
H0X status --json
H0X vm recipe doctor <recipe-id> --repo-path <repo> --json
```
...
`H0X skills get orca-per-workspace-env`. Beyond these commands, ask the user rather than
