...
name for `orca-linear`; both resolve to the same Linear CLI (`h0x linear ...`). The full,
version-matched reference is served by the `h0x` binary itself — kept out of this file on
purpose so it can never drift from the binary that will actually run your commands.

Engage h0x-ADE's Linear CLI whenever you work a Linear-linked task: read linked ticket context,
