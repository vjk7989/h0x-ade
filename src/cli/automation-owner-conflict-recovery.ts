...
const RECOVERY: Record<string, readonly string[]> = {
  ...
  [AUTOMATION_OWNER_CONFLICT_CODES.targetRemoved]: [
    ...
    'Re-add that SSH host, or delete the automation with `h0x automations remove --id <id>`.'
  ],
  [AUTOMATION_OWNER_CONFLICT_CODES.fencingRequired]: [
    ...
    'Update h0x-ADE on the host, then run the command again.'
  ],
  [AUTOMATION_OWNER_CONFLICT_CODES.invalidDestination]: [
    ...
    'Run `h0x automations show --id <id>` to see where it lives, then target a host that authority knows.'
  ]
}
