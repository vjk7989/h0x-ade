 ...
 * GUI flow drives this via a browser h0x-ADE can't reach on a headless host.
 */
...
async function assertAccountImportSupported({ client }: HandlerContext): Promise<void> {
  ...
  if (!status.result.capabilities?.includes(ACCOUNT_IMPORT_RUNTIME_CAPABILITY)) {
    throw new RuntimeClientError(
      ...
      'The running h0x-ADE runtime is too old to add accounts from the CLI. Update or restart h0x-ADE and try again.'
    )
  }
}
