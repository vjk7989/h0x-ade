...
describe('unknown command surfaces a suggestion', () => {
  ...
  it('prints did-you-mean for a near-miss command and exits non-zero', async () => {
    ...
    expect(stderr).toContain('h0x worktree')
  })
...
describe('unknown help command surfaces a suggestion', () => {
  it.each([
    ...
    expect(process.exitCode).toBe(1)
    expect(logSpy.mock.calls.flat().join('\n')).toContain('Did you mean: h0x worktree')
    logSpy.mockRestore()
    process.exitCode = 0
  })
})

describe('h0x root help', () => {
  ...
  it('advertises host-local account management', async () => {
    ...
    expect(logSpy.mock.calls.flat().join('\n')).toContain(
      'account add               Add a managed Claude or Codex account on this h0x-ADE host'
    )
    expect(logSpy.mock.calls.flat().join('\n')).toContain(
      'account list              List managed Claude and Codex accounts on this h0x-ADE host'
    )
    logSpy.mockRestore()
  })
  ...
  it('advertises computer-use capabilities discovery', async () => {
    ...
    expect(logSpy.mock.calls[0][0]).toContain(
      'orchestration worker-start Start a supervised worker locally or on a connected h0x-ADE server'
    )
    expect(logSpy.mock.calls[0][0]).toContain(
  ...
  it('progressively discloses Linear commands', async () => {
    ...
    expect(groupHelp).toContain('h0x linear')
    ...
    expect(issueHelp).toContain('h0x linear issue [<id>]')
    ...
    expect(searchHelp).toContain('h0x linear search <query>')
  ...
  it('hides removed parent-workspace help and scopes create parent selectors', async () => {
    ...
    expect(createHelp).toContain(
      '--no-parent only affects h0x-ADE lineage; omit --base-branch to use the repo default base'
    )
  ...
  it('distinguishes new worktrees from fresh agent terminals in command help', async () => {
    ...
    expect(terminalHelp).toContain('h0x terminal create --worktree active --command "codex" --json')
    expect(callMock).not.toHaveBeenCalled()
  })
})
