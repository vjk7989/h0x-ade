...
// Why: the SSH relay bridge executes this CLI on the h0x-ADE host while the
...
export async function main(
  ...
  if (argv.length === 1 && (argv[0] === '--version' || argv[0] === '-v')) {
    ...
    if (!version) {
      process.stderr.write('Could not determine the h0x-ADE version for this build.\n')
      process.exitCode = 1
      return
    }
  ...
  try {
    ...
    // lookup so users do not get misleading "h0x-ADE is not running" failures for
...
async function runClaudeTeams(argv: string[], cwd: string): Promise<void> {
  try {
    ...
    // h0x-ADE's own flag parser, so new Claude flags work without h0x-ADE changes.
