...
export function launchOrcaApp(): void {
  ...
  throw new RuntimeClientError(
    'runtime_open_failed',
    'Could not determine how to launch h0x-ADE. Start h0x-ADE manually and try again.'
  )
}
    ...
    const onClose = (code: number | null, signal: NodeJS.Signals | null): void => {
      ...
      finish(
        new RuntimeClientError(
          ...
          typeof code === 'number'
            ? `h0x-ADE serve exited before printing valid recipe JSON with code ${code}.`
            : `h0x-ADE serve exited before printing valid recipe JSON via ${signal}.`
        )
      )
    }
...
function resolveAppRoot(): string {
  ...
  // process.cwd(). Pin it to the app root so `h0x serve` behaves the same
...
function resolveForegroundOrcaExecutable(): string {
  ...
  throw new RuntimeClientError(
    'runtime_serve_failed',
    'Could not determine how to start h0x-ADE server. Set ORCA_APP_EXECUTABLE to the h0x-ADE executable.'
  )
}
