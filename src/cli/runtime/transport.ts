  ...
  return await new Promise((resolve, reject) => {
    ...
    if (!transport) {
      reject(
        new RuntimeClientError(
          'runtime_unavailable',
          'No compatible transport found in h0x-ADE runtime metadata.'
        )
      )
      return
    }
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new RuntimeClientError(
          'runtime_timeout',
          'Timed out waiting for the h0x-ADE runtime to respond.'
        )
      )
    }, timeoutMs)
    ...
    socket.once('error', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'Could not connect to the running h0x-ADE app. Restart h0x-ADE and try again.'
        )
      })
    })
    ...
    socket.on('data', (chunk: string) => {
      ...
      while (newlineIndex !== -1 && !settled) {
        ...
        try {
          ...
          finish({
            ...
            error: new RuntimeClientError(
              ...
              'The h0x-ADE runtime returned an invalid response frame.'
            )
          })
          return
        }
        ...
        // setTimeout().refresh() is stable since Node 10 (h0x-ADE ships on
        ...
        // the decode boundary so version skew between the CLI and the h0x-ADE
        ...
        if (response.id !== requestId) {
          finish({
            ...
            error: new RuntimeClientError(
              'invalid_runtime_response',
              'The h0x-ADE runtime returned a mismatched response id.'
            )
          })
          return
        }
        if (response._meta?.runtimeId && response._meta.runtimeId !== metadata.runtimeId) {
          finish({
            ...
            error: new RuntimeClientError(
              'runtime_unavailable',
              'The h0x-ADE runtime changed while the request was in flight. Retry the command.'
            )
          })
          return
        }
