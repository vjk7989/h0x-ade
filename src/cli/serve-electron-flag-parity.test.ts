...
// It fails when `h0x serve` grows a flag that a direct `<binary> serve …` launch would silently drop (#12677).
...
describe('serve flag parity between the CLI spec and the Electron argv rewrite', () => {
  ...
  it.each(translatedFlags)('rewrites CLI-form --%s into the --serve-* form', (flag) => {
    ...
    if (takesValue) {
      // The equals form is the other shape `h0x serve` accepts; normalize it to the internal shape.
    ...
    // Idempotent: `h0x serve` spawns the app already in this shape, and the rewrite runs over it too.
