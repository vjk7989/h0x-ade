...
export const CORE_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['open'],
    summary: 'Launch h0x-ADE and wait for the runtime to be reachable',
    usage: 'h0x open [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    examples: ['h0x open', 'h0x open --json']
  },
  ...SERVE_COMMAND_SPECS,
  {
    path: ['status'],
    summary: 'Show app/runtime/graph readiness',
    usage: 'h0x status [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    examples: ['h0x status', 'h0x status --json']
  },
  {
    path: ['claude-teams'],
    ...
    notes: [
      ...
      'Must be run from inside an h0x-ADE terminal. Starts Claude Code Agent Teams in the current pane and opens teammates as native h0x-ADE splits.'
    ],
    examples: ['h0x claude-teams', 'h0x claude-teams --resume <session-id>']
  },
  {
    path: ['repo', 'list'],
    summary: 'List repos registered in h0x-ADE',
    usage: 'h0x repo list [--json]',
    allowedFlags: [...GLOBAL_FLAGS]
  },
  {
    path: ['repo', 'add'],
    summary: 'Add a project to h0x-ADE by filesystem path',
    usage: 'h0x repo add --path <path> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'path']
  },
  {
    path: ['repo', 'show'],
    summary: 'Show one registered repo',
    usage: 'h0x repo show --repo <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo']
  },
  {
    path: ['repo', 'set-base-ref'],
    summary: "Set the repo's default base ref for future worktrees",
    usage: 'h0x repo set-base-ref --repo <selector> --ref <ref> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo', 'ref']
  },
  {
    path: ['repo', 'search-refs'],
    summary: 'Search branch/tag refs within a repo',
    usage: 'h0x repo search-refs --repo <selector> --query <text> [--limit <n>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'repo', 'query', 'limit']
  },
  {
    path: ['worktree', 'list'],
    summary: 'List h0x-ADE-managed worktrees',
    usage: 'h0x worktree list [--repo <selector>] [--limit <n>] [--json]',
  ...
  },
  {
    ...
    usage: 'h0x worktree show --worktree <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree']
  },
  {
    path: ['worktree', 'current'],
    summary: 'Show the h0x-ADE-managed worktree for the current directory',
    usage: 'h0x worktree current [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Resolves the current shell directory to a path: selector so agents can target the enclosing h0x-ADE worktree without spelling out $PWD.'
    ],
    examples: ['h0x worktree current', 'h0x worktree current --json']
  },
  {
    path: ['worktree', 'create'],
    summary: 'Create a new h0x-ADE-managed worktree',
    usage:
    ...
    notes: [
      'This creates a new checkout. For a fresh agent in an existing worktree, use `h0x terminal create --worktree active --command "codex"` instead.',
      'By default, h0x-ADE records the new worktree as a child of the caller context when it can infer one from the h0x-ADE terminal or current directory.',
      'If --repo is omitted, h0x-ADE infers the repo from the current h0x-ADE-managed worktree.',
      'Use --project with --host to create on a ready project host setup without spelling the backing repo id.',
      '--host runtime:<environment-id> creates on that paired h0x-ADE server; use the id from `h0x environment list`, not the environment name.',
      'For related work, use the inferred parent or pass --parent-worktree active, folder:<id>, or worktree:<worktreeId> to make the relationship explicit. Worktree ids are the full <repo-id>::<path> values returned by `h0x worktree list --json`.',
      'Use --no-parent when the new worktree should be independent of the current context.',
      '--no-parent only affects h0x-ADE lineage; omit --base-branch to use the repo default base, or pass the default base ref explicitly for independent top-level work.',
      'By default this creates the worktree and its first terminal without switching the active h0x-ADE view.',
    ...
    examples: [
      ...
      'h0x worktree create --repo id:<repoId> --name independent-task --no-parent --json'
    ]
  },
  {
    path: ['worktree', 'set'],
    summary: 'Update h0x-ADE metadata for a worktree',
    usage:
    ...
    examples: [
      'h0x worktree set --worktree active --linear-issue STA-335 --json',
      'h0x worktree set --worktree active --linear-issue null --json'
    ]
  },
  {
    path: ['worktree', 'rm'],
    ...
    destructive: true,
    summary: 'Remove a worktree from h0x-ADE and git',
    usage: 'h0x worktree rm --worktree <selector> [--force] [--run-hooks] [--json]',
    ...
    notes: [
      ...
      'For Git worktrees, removal also attempts to delete the checked-out local branch, with or without --force. h0x-ADE retains branches it knows predated the worktree and any branch whose changes it cannot prove are already merged.'
    ]
  },
  {
    path: ['worktree', 'ps'],
    summary: 'Show a compact orchestration summary across worktrees',
    usage: 'h0x worktree ps [--limit <n>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'limit'],
    notes: [...WORKTREE_LISTING_SCOPE_NOTES]
  },
  {
    path: ['terminal', 'list'],
    summary: 'List live h0x-ADE-managed terminals',
    usage:
  ...
  },
  {
    ...
    usage: 'h0x terminal show [--terminal <handle>] [--json]',
  ...
  },
  {
    ...
    usage:
      'h0x terminal read [--terminal <handle>] [--cursor <n>] [--limit <n>] [--screen] [--json]',
    ...
    examples: [
      ...
      'h0x terminal read --terminal term_abc123 --cursor 42 --limit 1000 --json',
      'h0x terminal read --terminal term_abc123 --screen --json'
    ]
  },
  {
    path: ['terminal', 'send'],
  ...
  },
  {
    ...
    usage: 'h0x terminal stop --worktree <selector> [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'worktree'],
    notes: [
  ...
  },
  {
    ...
    examples: [
      ...
      'h0x terminal create --worktree path:/projects/myapp --title "RUNNER" --command "opencode"',
      'h0x terminal create --worktree path:/projects/myapp --command "opencode" --focus'
    ]
  },
  {
    ...
    usage: 'h0x terminal switch [--terminal <handle>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal'],
    examples: ['h0x terminal switch --terminal term_abc123']
  },
  TERMINAL_CLOSE_COMMAND_SPEC,
  {
    path: ['terminal', 'rename'],
    summary: 'Set or clear the title of a terminal tab',
    usage: 'h0x terminal rename [--terminal <handle>] [--title <text>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal', 'title'],
    notes: ['Omit --title or pass an empty string to reset to the auto-generated title.'],
    examples: [
      'h0x terminal rename --terminal term_abc123 --title "RUNNER"',
      'h0x terminal rename --terminal term_abc123 --json'
    ]
  },
  {
    path: ['terminal', 'split'],
    ...
    usage:
      'h0x terminal split [--terminal <handle>] [--direction horizontal|vertical] [--command <text>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'terminal', 'direction', 'command'],
    examples: [
      'h0x terminal split --terminal term_abc123 --direction horizontal --json',
      'h0x terminal split --terminal term_abc123 --command "codex"'
    ]
  }
]
