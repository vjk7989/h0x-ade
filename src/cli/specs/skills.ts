...
export const SKILL_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    usage: 'h0x skills installed [--json]',
    ...
    notes: [
      ...
      'Use an exact ID or an unambiguous name with `h0x skills share --skill <selector>`.'
    ]
  },
  {
    path: ['skills', 'share'],
    ...
    usage:
      'h0x skills share --skill <selector> [--skill <selector> ...] --bundle-name <name> ' +
    ...
    notes: [
      ...
      'The bundle name may be human-readable; h0x-ADE converts it to a portable lowercase package name.',
      'Selectors are exact discovery IDs or unambiguous names from `h0x skills installed`.',
    ...
    examples: [
      'h0x skills share --skill frontend --bundle-name "Frontend Skills"',
      'h0x skills share --skill frontend --skill testing --bundle-name "Team Toolkit" --json'
    ]
  },
  {
    path: ['skills', 'list'],
    summary: 'List version-matched skill guides bundled with this h0x CLI',
    usage: 'h0x skills list [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Reads bundled guide metadata locally without contacting the h0x-ADE runtime.',
      'With --json, prints a topics array of canonical names and one-line descriptions.',
      'Use `h0x skills get <name>` for the full guide, or `h0x skills install` to install skills.'
    ]
  },
  {
    path: ['skills', 'get'],
    ...
    usage: 'h0x skills get <topic> [--full] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'topic', 'full'],
    positionalArgs: ['topic'],
    notes: [
      'Reads bundled guide content locally without contacting the h0x-ADE runtime.',
    ...
    examples: ['h0x skills get orca-cli', 'h0x skills get orchestration --full']
  },
  {
    path: ['skills', 'install'],
    summary: 'Install bundled h0x-ADE skills via the community skills CLI',
    usage:
      'h0x skills install [--skill <name>]... [--all] [--agent <name>[,<name>]] ' +
      '[--local] [--dry-run] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'skill', 'all', 'agent', 'local', 'dry-run'],
    notes: [
      'Reads the bundled skill registry locally without contacting the h0x-ADE runtime.',
      'Resolves to the same `npx skills add <repo> --skill <name> ...` command used by ' +
        'h0x-ADE Settings, plus the non-interactive flags an unattended host needs ' +
      ...
      'Targets the coding agents h0x-ADE detects on this host, plus the shared ' +
      ...
      'Use --agent <name>[,<name>...] to choose targets yourself, or --agent universal ' +
        'for the shared directory alone. Required when h0x-ADE detects no agent.',
    ...
    examples: [
      ...
      'h0x skills install --skill orca-cli --local',
  ...
  {
    path: ['skills', 'update'],
    summary: 'Update already-installed h0x-ADE skills via the community skills CLI',
    usage: 'h0x skills update [--skill <name>]... [--all] [--local] [--dry-run] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'skill', 'all', 'local', 'dry-run'],
    notes: [
      'Reads the bundled skill registry locally without contacting the h0x-ADE runtime.',
      'Resolves to the same `npx skills update <names...>` command used by h0x-ADE Settings, ' +
      ...
      'Only refreshes skills that are already installed; use `h0x skills install` first.',
      'Use --dry-run to print the resolved command without running it.',
    ...
    examples: [
      'h0x skills update',
      'h0x skills update --skill orca-cli --skill orchestration',
      'h0x skills update --skill orca-cli --local',
      'h0x skills update --all --dry-run'
    ]
  }
]
