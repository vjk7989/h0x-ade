...
export function describeStrandedAutomationRun(run: AutomationRun): string {
  if (run.status === 'dispatching') {
    return 'h0x-ADE stopped before this run reported that its agent started.'
  }
  return 'h0x-ADE lost the terminal for this run before it reported completion.'
}
...
function describeObservationError(error: unknown): string {
  ...
  return 'h0x-ADE stopped watching this run before it reported completion.'
}
