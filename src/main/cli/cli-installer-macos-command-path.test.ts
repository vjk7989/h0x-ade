...
describe('CliInstaller', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const absentUsrLocalBin = join(fixture.root, 'usr', 'local', 'bin', 'h0x')
      ...
      expect(status.commandPath).toBe(join(homePath, '.local', 'bin', 'h0x'))
      expect(status.state).toBe('not_installed')
      expect(status.supported).toBe(true)

      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandPath).toBe(join(homePath, '.local', 'bin', 'h0x'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const installPath = join(usrLocalBin, 'h0x')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: true,
        resourcesPath,
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const defaultInstallPath = join(usrLocalBin, 'h0x')
      const userInstallPath = join(userLocalBin, 'h0x')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const userInstallPath = join(userLocalBin, 'h0x')
      const launcherPath = join(resourcesPath, 'bin', 'h0x')
      const oldLauncherPath = join(fixture.root, 'Old.app', 'Contents', 'Resources', 'bin', 'h0x')
      await mkdir(usrLocalBin, { recursive: true })
      await mkdir(userLocalBin, { recursive: true })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const status = await installer.getStatus()
      expect(status.commandName).toBe('h0x')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(s1.commandPath).toBe(join(homePath, '.local', 'bin', 'h0x'))
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const bundledLauncher = join(resourcesPath, 'bin', 'h0x')
