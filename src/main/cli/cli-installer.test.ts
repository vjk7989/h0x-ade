...
async function fakeAppImageExtractRunner(_appImagePath: string, cwd: string): Promise<void> {
  ...
  await writeFile(join(launcherDir, 'h0x'), '#!/usr/bin/env bash\n', {
    encoding: 'utf8',
    mode: 0o755
  })
}
...
describe('CliInstaller', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const installPath = join(fixture.root, 'bin', 'h0x')
      ...
      expect(initial.launcherPath).toContain(join('userData', 'cli', 'bin', 'h0x'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const fixture = await makeFixture()
      const installPath = join(fixture.root, '.local', 'bin', 'h0x')
      const installer = new CliInstaller({
        platform: 'linux',
        isPackaged: false,
        userDataPath: fixture.userDataPath,
        execPath: '/opt/Orca/h0x',
        appPath: fixture.appPath,
        commandPathOverride: installPath,
      ...
      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandName).toBe('h0x')
  ...
  // packaged `orca` / `h0x` commands developers rely on day to day.
  it.skipIf(process.platform === 'win32')(
    'uses a separate h0x-dev command for default development installs',
    async () => {
      const fixture = await makeFixture()
      ...
      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandName).toBe('h0x-dev')
      expect(installed.commandPath).toBe(join(commandDir, 'h0x-dev'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const installPath = join(commandDir, 'h0x')
  ...
  it.skipIf(process.platform === 'win32')(
    'removes the old managed linux orca symlink when installing h0x',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const commandDir = join(homePath, '.local', 'bin')
      ...
      const installed = await installer.install()
      expect(installed.commandPath).toBe(join(commandDir, 'h0x'))
  ...
  it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)(
    ...
    async () => {
      ...
      const installPath = join(protectedDir, 'bin', 'h0x')
