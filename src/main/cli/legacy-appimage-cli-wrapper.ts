...
const APPIMAGE_CLI_SCRIPT = [
  ...
  'if(!appDir){console.error("h0x-ADE AppImage runtime did not set APPDIR.");process.exit(1);}',
...
if [ ! -f "$APPIMAGE" ]; then
  echo "h0x-ADE AppImage not found at $APPIMAGE" >&2
  echo "If you moved the AppImage, re-run CLI registration from h0x-ADE Settings." >&2
  exit 1
fi
