...
async function makeFixture(): Promise<{ userDataPath: string; resourcesPath: string }> {
  ...
  // The bundled h0x launcher must exist for the shim to be written.
  mkdirSync(join(resourcesPath, 'bin'), { recursive: true })
  writeFileSync(join(resourcesPath, 'bin', 'h0x'), '#!/usr/bin/env bash\n', 'utf8')
...
describe('ensureLinuxTerminalOrcaCliShimDir', () => {
  it('uses the mounted bundled launcher when only APPDIR is inherited', async () => {
    ...
    expect(shimDir).toBe(join(userDataPath, 'linux-h0x-cli-shim'))
    expect(readFileSync(join(shimDir!, 'h0x'), 'utf8')).toContain(
      `exec '${join(resourcesPath, 'bin', 'h0x')}' "$@"`
    )
  })

  it('writes an executable bare-orca shim that execs the bundled h0x launcher', async () => {
    const { userDataPath, resourcesPath } = await makeFixture()
    ...
    expect(shimDir).toBe(join(userDataPath, 'linux-h0x-cli-shim'))
    const content = readFileSync(join(shimDir!, 'h0x'), 'utf8')
    ...
    if (process.platform !== 'win32') {
      const mode = statSync(join(shimDir!, 'h0x')).mode & 0o777
  ...
  it('reuses the shim path and re-asserts its exec bit', async () => {
    ...
    mkdirSync(join(otherUserData, 'linux-h0x-cli-shim'), { recursive: true })
    writeFileSync(join(otherUserData, 'linux-h0x-cli-shim', 'h0x'), 'stale contents', 'utf8')
    chmodSync(join(otherUserData, 'linux-h0x-cli-shim', 'h0x'), 0o644)
    ...
    const healedPath = join(healed!, 'h0x')
    expect(readFileSync(healedPath, 'utf8')).toContain('h0x')
    if (process.platform !== 'win32') {
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      ...
      await writeFile(appImagePath, '#!/usr/bin/env bash\n', { encoding: 'utf8', mode: 0o755 })
      const cacheRootPath = join(userDataPath, 'cache')
      const firstLauncher = join(resourcesPath, 'bin', 'h0x')
      ...
      const nextLauncher = join(nextResourcesPath, 'bin', 'h0x')
  ...
  it.skipIf(!canFenceAppImageRuntime)(
    ...
    async () => {
      ...
      await mkdir(userDataPath, { recursive: true })
      await writeFile(appImagePath, '#!/usr/bin/env bash\n', { mode: 0o755 })
      const liveLauncher = join(resourcesPath, 'bin', 'h0x')
  ...
  it('returns null (and does not memoize) when the bundled launcher is missing', async () => {
    ...
    expect(recovered).toBe(join(userDataPath, 'linux-h0x-cli-shim'))
  })
})
