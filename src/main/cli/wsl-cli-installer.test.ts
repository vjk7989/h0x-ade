...
function makeHostStatus(
  launcherPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\Orca\\resources\\bin\\h0x.exe'
) {
  return {
    platform: 'win32',
    commandName: 'h0x',
...
ORCA_WIN_LAUNCHER='C:\\Program Files\\Orca\\resources\\bin\\h0x.cmd'
ORCA_BRIDGE_PS1='/home/alice/.local/share/h0x/h0x-wsl-bridge.ps1'
...
function createWslRunner(
  ...
  const commandPath = '/home/alice/.local/bin/h0x'
  const legacyCommandPath = '/home/alice/.local/bin/orca'
  const bridgePath = '/home/alice/.local/share/h0x/h0x-wsl-bridge.ps1'
  ...
  const runner = vi.fn(async (_distro: string, command: string) => {
    ...
    if (command.includes('cat > "$command_tmp"')) {
      ...
      if (
        ...
        !files.get(bridgePath)?.includes('# h0x-ADE managed WSL CLI PowerShell bridge')
      ) {
...
describe('WslCliInstaller', () => {
  ...
  it('installs a WSL launcher that forwards to the Windows Orca launcher', async () => {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'not_installed',
      commandPath: '/home/alice/.local/bin/h0x'
    })
    ...
    expect(wsl.getFile()).toBe(
      _internals.buildWslLauncher(
        'C:\\Users\\me\\AppData\\Local\\Programs\\Orca\\resources\\bin\\h0x.exe',
        '/home/alice/.local/share/h0x/h0x-wsl-bridge.ps1'
      )
    )
  ...
  it('derives the shared WSL bridge path for current and legacy command names', () => {
    expect(_internals.getBridgePathFromCommandPath('/home/alice/.local/bin/h0x')).toBe(
      '/home/alice/.local/share/h0x/h0x-wsl-bridge.ps1'
    )
  ...
  it('reports installed WSL launchers whose bin directory is missing from PATH', async () => {
    ...
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
      hostInstaller: { getStatus: async () => makeHostStatus('C:\\Orca\\h0x.cmd') },
      wslRunner: wsl.runner
    })
  ...
  it('accepts current managed WSL scripts with an extra heredoc trailing newline', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      wslRunner: async (distro, command) => {
        if (command.includes('cat /home/alice/.local/share/h0x/h0x-wsl-bridge.ps1')) {
  ...
  it('generates a launcher that forwards arguments through a PowerShell file bridge', () => {
    const launcher = _internals.buildWslLauncher(
      'C:\\Program Files\\Orca\\resources\\bin\\h0x.exe',
      '/home/alice/.local/share/h0x/h0x-wsl-bridge.ps1'
    )
    ...
    expect(launcher).toContain(
      'h0x-ADE WSL CLI requires Windows interop and could not find powershell.exe.'
    )
  ...
  it('marks stale managed launchers that point at the old app bin instead of packaged resources', async () => {
    const oldLauncher = _internals.buildWslLauncher(
      'C:\\Users\\me\\AppData\\Local\\Programs\\Orca\\bin\\h0x.cmd',
      '/home/alice/.local/share/h0x/h0x-wsl-bridge.ps1'
    )
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'stale',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\Orca\\bin\\h0x.cmd',
      launcherPath: 'C:\\Users\\me\\AppData\\Local\\Programs\\Orca\\resources\\bin\\h0x.exe'
    })

    await expect(installer.install()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\Orca\\resources\\bin\\h0x.exe'
    })
  })

  it('repairs the frozen pre-rc4 registration so orchestration send/reply reach native rc4', async () => {
    const nativeLauncher = 'C:\\Program Files\\Orca\\resources\\bin\\h0x.exe'
  ...
  it('repairs a managed launcher whose bridge is missing, but preserves a conflicting bridge', async () => {
    const nativeLauncher = 'C:\\Orca\\resources\\bin\\h0x.exe'
    ...
    const staleBridge = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER, true, {
      initialBridge: '# h0x-ADE managed WSL CLI PowerShell bridge\nWrite-Output "stale"\n'
    })
  ...
  it('moves a legacy-only managed registration to h0x without touching unmanaged names', async () => {
    const nativeLauncher = 'C:\\Orca\\resources\\bin\\h0x.exe'
  ...
  it('keeps the pre-rc4 files on a transactional replacement failure', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      distro: 'Ubuntu',
      hostInstaller: {
        getStatus: async () => makeHostStatus('C:\\Program Files\\Orca\\resources\\bin\\h0x.exe')
      },
      wslRunner: wsl.runner
    })
    ...
    // would leave a window where a concurrent shell finds no h0x at all.
    expect(installCommand).not.toContain('command_backup')
    expect(installCommand).not.toContain(`mv -f '/home/alice/.local/bin/h0x'`)
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const commandPath = join(home, '.local', 'bin', 'h0x')
      const bridgePath = join(home, '.local', 'share', 'h0x', 'h0x-wsl-bridge.ps1')
      const bridge = _internals.buildWslBridgeScript()
      await mkdir(join(home, '.local', 'bin'), { recursive: true })
      await mkdir(join(home, '.local', 'share', 'h0x'), { recursive: true })
  ...
  it('is idempotent after repairing an old managed registration', async () => {
    const nativeLauncher = 'D:\\Custom Orca\\resources\\bin\\h0x.exe'
    const wsl = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER)
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
    ...
    expect(wsl.getFile()).toContain("ORCA_WIN_LAUNCHER='D:\\Custom Orca\\resources\\bin\\h0x.exe'")
  })
