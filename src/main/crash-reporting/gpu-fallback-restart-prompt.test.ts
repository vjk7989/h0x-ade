...
describe('promptForGpuFallbackRestart', () => {
  it('offers a restart without forcing it', async () => {
    ...
    expect(showMessageBoxMock).toHaveBeenCalledWith(parentWindow, {
      ...
      defaultId: 0,
      cancelId: 1,
      title: 'Restart h0x-ADE in Safe Graphics Mode?',
      message: "h0x-ADE's graphics process has crashed repeatedly.",
      detail:
        'Safe graphics mode disables hardware acceleration and WebGL for this h0x-ADE version. Terminals and 3D content may render more slowly. Keep Running leaves graphics settings unchanged.'
    })
  })
