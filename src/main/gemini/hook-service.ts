...
// Why: Gemini has no permission-prompt hook (approvals are inline UI), so h0x-ADE can't show a waiting state — upstream limitation.
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    ...
    // Why: source refreshes endpoint coords so a PTY surviving an h0x-ADE restart keeps reporting. See claude/hook-service.ts.
...
export class GeminiHookService {
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    try {
      ...
      // Why: SSH remotes always use POSIX `.sh` paths even when h0x-ADE runs on Windows.
