...
export const LINUX_PACKAGE_MARKER_UNUSABLE_MESSAGE =
  'h0x-ADE could not verify the installed Linux package format, so it will not install this update automatically. Download the update from the official release page and install it manually.'
export const LINUX_PACKAGE_EXTERNALLY_MANAGED_MESSAGE =
  'This copy of h0x-ADE is managed by your system package manager, so h0x-ADE cannot install updates itself. Update h0x-ADE through your distribution instead.'
export const LINUX_PACKAGE_MANUAL_INSTALL_MESSAGE =
  'Quit h0x-ADE before running the system package install command.'
const PACKAGE_METADATA_UNUSABLE_MESSAGE =
  'The downloaded package metadata could not be verified. Quit h0x-ADE before downloading and installing the update from the official release page.'
