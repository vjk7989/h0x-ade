...
function buildAndApplyMenu(options: RegisterAppMenuOptions): void {
  ...
  const featureTourItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.exploreOrca', 'Explore h0x-ADE'),
    click: (_menuItem, window) => onOpenFeatureTour(window)
  }

  const setupGuideItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.gettingStarted', 'Getting Started with h0x-ADE'),
  ...
  // redundant "h0x-ADE" entry with roles that don't apply, so we omit it there
    ...
    submenu: [
      ...
      }),
      {
        ...
        click: () => {
          ...
          // control, so raw Electron paste cannot know which h0x-ADE surface owns it.
  ...
  const appearanceSubmenu: Electron.MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      {
        label: translateMain('menu.showMobileButton', 'Show h0x-ADE Mobile Button'),
        type: 'checkbox',
