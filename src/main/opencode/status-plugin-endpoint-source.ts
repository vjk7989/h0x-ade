export function getStatusPluginEndpointSource(): string[] {
  return [
    ...
    "// OpenCode's Node process (not h0x-ADE's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on h0x-ADE restart',
    ...
    '      // writeEndpointFile on the h0x-ADE side) allocates a fresh inode on',
    ...
    '  // frozen when OpenCode was fork()ed — stale after an h0x-ADE restart. The',
    '  // file is rewritten on every h0x-ADE start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / h0x-ADE',
