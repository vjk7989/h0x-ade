...
function devSelf(): OrcaOrgMember {
  return {
    ...
    displayName: cleanEnvString(process.env.ORCA_CLOUD_DEV_DISPLAY_NAME, 'h0x-ADE Dev'),
    role: 'owner'
  }
}
...
function seedDevRoster(): DevOrgRoster {
  return {
    members: [
      ...
      // Why: userId null exercises the "hasn't signed in to h0x-ADE yet" disabled row.
