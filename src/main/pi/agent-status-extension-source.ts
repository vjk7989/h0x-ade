...
// any h0x-ADE dep into the pi runtime.
...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  // so it cannot import from h0x-ADE's main bundle. fs/http coords come from
  // the same endpoint file the OpenCode plugin reads (process.env is frozen
  // at PTY spawn, so on h0x-ADE restart we have to re-read it from disk).
  return [
    ...
    '// h0x-ADE receiver from building an unbounded queue of obsolete snapshots.',
    ...
    '    timeout = setTimeout(() => {',
    '      controller?.abort()',
    "      reject(new Error('h0x-ADE hook delivery timed out'))",
    ...
    '    ])',
    '  } catch {',
    '    // Why: status reporting must never fail the pi run just because h0x-ADE',
    '    // is unavailable or the loopback request failed (e.g. h0x-ADE restart).',
