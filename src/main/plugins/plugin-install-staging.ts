...
async function readInstallManifest(
  ...
  if (!satisfiesOrcaEngineRange(hostVersion, parsed.manifest.engines.orca)) {
    return {
      ok: false,
      error: `plugin requires h0x-ADE ${parsed.manifest.engines.orca} (this is ${hostVersion})`
    }
  }
...
export async function installStagedPluginTree(input: {
  ...
  if (blockedReason) {
    return { ok: false, error: `plugin is blocked by h0x-ADE's safety list: ${blockedReason}` }
  }
