/**
 * Content of the bash rcfile h0x-ADE launches interactive bash with.
 *
...
export function getBashShellReadyRcfileContent(): string {
  return `# h0x-ADE bash shell-ready wrapper
...
# PATH edits, hooks, and prompt init in h0x-ADE startup-command shells.
...
__orca_restore_agent_teams_path
# Why: user startup files may set the default OpenCode config after h0x-ADE's
# spawn env; restore the h0x-ADE-managed config dir before the first prompt.
[[ -n "\${ORCA_OPENCODE_CONFIG_DIR:-}" ]] && export OPENCODE_CONFIG_DIR="\${ORCA_OPENCODE_CONFIG_DIR}"
[[ -n "\${ORCA_MIMOCODE_HOME:-}" ]] && export MIMOCODE_HOME="\${ORCA_MIMOCODE_HOME}"
${getPosixOmpShellWrapper()}
# Why: Codex must keep using h0x-ADE's runtime CODEX_HOME after profile scripts.
