...
export function buildLocalPtySpawnEnvironment(args: {
  ...
  const spawnEnv: Record<string, string> = {
    ...
    // Why: force OSC 8 detection because xterm.js parses those links natively.
    FORCE_HYPERLINK: '1'
  } as Record<string, string>
  // Why: h0x-ADE can be launched from an h0x-ADE terminal; pane identity belongs to the child PTY, not the parent shell.
