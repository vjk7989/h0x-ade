...
// Why: h0x-ADE never runs grok login; it only reads the session file the CLI updates.
export async function fetchGrokRateLimits(
  ...
  if (!isGrokAccessTokenFresh(session)) {
    ...
    return result(
      'error',
      'Grok sign-in expired — run grok on the computer running h0x-ADE; sign in if prompted. No chat message is needed.',
