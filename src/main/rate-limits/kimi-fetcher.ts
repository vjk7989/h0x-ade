...
// The base URL is overridable via the same env var the CLI honours so h0x-ADE
...
function expiredSessionMessage(home: KimiHomeResolution): string {
  const where =
    home.runtime === 'wsl'
      ...
      : 'on the computer running h0x-ADE'
 ...
 * h0x-ADE must NEVER refresh or rewrite that file — a rotated refresh token would
