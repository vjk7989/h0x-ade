...
function precommitRefusal(error: unknown): AgentSessionWireRefusal {
  ...
  if (code) {
    return { code, message: 'h0x-ADE cannot open a structured agent chat for this workspace.' }
  }
  ...
  return {
    ...
    message: `h0x-ADE could not prepare a structured agent chat for this workspace: ${message}`
  }
}
