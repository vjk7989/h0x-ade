...
describe('runtime RPC startup failure reporting', () => {
  ...
  it('shows the CLI impact and local cause', async () => {
    ...
    expect(showMessageBoxMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        type: 'error',
        title: 'h0x CLI unavailable',
        message: "h0x-ADE couldn't start its local command transport.",
        detail: expect.stringMatching(
          /h0x status.*h0x terminal.*orchestration.*Cause: metadata write failed/s
        )
      })
    )
  })
  ...
  it.each([
    ['EACCES', "Check permissions on h0x-ADE's data folder"],
    ['EPERM', "Check permissions on h0x-ADE's data folder"],
    ...
    ['ENOENT', "h0x-ADE's data folder may be missing"],
  ...
  it('falls back to a plain restart when the cause is unrecognised', async () => {
    ...
    const detail = showMessageBoxMock.mock.calls[0]?.[1]?.detail as string
    expect(detail).toContain('Restart h0x-ADE to try again.')
    expect(detail).not.toContain("Check permissions on h0x-ADE's data folder")
  })
