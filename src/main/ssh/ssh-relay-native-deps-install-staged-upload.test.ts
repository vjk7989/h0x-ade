...
describe('installNativeDeps staged uploads', () => {
  ...
  it('names the fix when node-gyp cannot download headers and the host ships none (STA-6674)', async () => {
    ...
    expect((error as Error).message).not.toContain('h0x-ADE defect')
  ...
  it('reports an h0x-ADE defect when headers were exported but node-gyp downloaded anyway', async () => {
    ...
    expect((error as Error).message).toContain('h0x-ADE defect')
