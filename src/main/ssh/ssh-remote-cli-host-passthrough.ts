...
// switch (the cause of "Unsupported SSH h0x CLI command", #7716), the host
...
// Why: only h0x-ADE terminal-context vars may cross from the remote shell into
...
export async function runHostOrcaCliPassthrough(
  ...
  try {
    ...
    // reports "h0x-ADE is not running" against a healthy app.
  ...
  if (!entryExists(cliEntryPath)) {
    throw new HostCliUnavailableError(`h0x CLI entry not found at ${cliEntryPath}`)
  }
  ...
  return await new Promise<RemoteOrcaCliResult>((resolve, reject) => {
    ...
    const killTimer = setTimeout(() => {
      ...
      resolve({
        ...
        stderr: `${stderr.toString()}h0x CLI bridge timed out after ${killTimeoutMs}ms on the host.\n`,
        exitCode: 1
      })
    }, killTimeoutMs)
    killTimer.unref?.()
    ...
    child.on('error', (err) => {
      ...
      reject(
        new HostCliUnavailableError(`Failed to launch the h0x CLI on the host: ${err.message}`)
      )
    })
