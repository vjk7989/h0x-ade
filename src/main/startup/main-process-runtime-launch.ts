...
function installRuntimeRpc(
  ...
  const runtimeRpc = new OrcaRuntimeRpcServer({
    ...
    // `h0x serve` is an explicit remote opt-in, and E2E keeps the wide bind its harness connects over.
    ...
    ...(serveOptions?.wsPort !== undefined
      ? {
          ...
          // Why: only explicit `h0x serve --port` overrides a stale STA-1511 fallback (issue #8535); default/dev stay fallback-first for pairing stability.
