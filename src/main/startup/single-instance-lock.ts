...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another h0x-ADE instance is already running for this userData profile; exiting this launch after requesting the existing window. If no h0x-ADE process is running, this may be an Electron/macOS single-instance lock failure.'
...
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] ORCA_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another h0x-ADE instance running for the same profile.'
...
// Why: a duplicate `h0x serve` is a supervisor artifact, not a user asking for a window; fail open when argv is unavailable.
...
/**
 * Why: h0x-ADE writes two canonical discovery files into `<userData>/`:
 ...
 * instance quits, metadata points at a dead pid and `h0x status` reports
 ...
 * way dev (`h0x-dev` userData) and packaged (`h0x-ADE` userData) runs lock in
