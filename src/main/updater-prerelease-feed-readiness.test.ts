...
function buildAtomFeed(tags: string[]): string {
  return `<?xml version="1.0" encoding="UTF-8"?><feed>${tags
    .map(
      (tag) =>
        `<entry><link rel="alternate" type="text/html" href="https://github.com/vjk7989/h0x-ade/releases/tag/${tag}"/><title>${tag}</title></entry>`
    )
    .join('')}</feed>`
}
...
function buildWindowsManifest(version: string): string {
  return [
    `version: ${version}`,
    'files:',
    '  - url: h0x-windows-setup.exe',
    '    sha512: test',
    'path: h0x-windows-setup.exe'
  ].join('\n')
}
...
function respondWithAtom(
  ...
  netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
    if (url === 'https://github.com/vjk7989/h0x-ade/releases.atom') {
...
describe('fetchNewerReleaseTagsWithReadiness', () => {
  ...
  it('requires every asset referenced by the manifest files list to be reachable', async () => {
    netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
      ...
      if (init?.method === 'HEAD') {
        ...
        const missing = latest && url.endsWith('/h0x-windows-setup.exe')
        return Promise.resolve({
  ...
  it('treats an explicit asset 404 as not-ready when another asset is unavailable', async () => {
    netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
      ...
      if (init?.method === 'HEAD') {
        const isWindowsAsset = url.endsWith('/h0x-windows-setup.exe')
        return Promise.resolve({
          ok: false,
