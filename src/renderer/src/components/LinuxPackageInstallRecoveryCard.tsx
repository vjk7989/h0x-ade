...
function copiedNote(packageFileName: string): string {
  return translate(
    ...
    'Command copied. Quit h0x-ADE, run it in a system terminal to install {{value0}}, then reopen h0x-ADE.',
    {
...
export function LinuxPackageInstallRecoveryCard({
  ...
  const EXPLAINER = translate(
    ...
    'Copy the command, quit h0x-ADE, and run it in a system terminal on the computer where h0x-ADE is installed. Reopen h0x-ADE after it finishes.'
  )
  ...
  const TRUST_NOTE = translate(
    ...
    'h0x-ADE checks the downloaded file against the release metadata at the moment it builds this command. The system package itself is not signature-checked, and h0x-ADE cannot vouch for the file after that point.'
  )
