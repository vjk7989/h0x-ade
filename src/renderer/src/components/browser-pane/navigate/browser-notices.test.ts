...
describe('browser notice formatting', () => {
  it('formats denied permissions with safe copy', () => {
    ...
    ).toBe('https://example.com asked for camera or microphone access, and h0x-ADE denied it.')
    expect(
      formatPermissionNotice({
        browserPageId: 'browser-1',
    ...
    ).toBe('this page asked for your location, and h0x-ADE denied it.')
  })
  ...
  it('names the storage permission in words rather than its raw token', () => {
    ...
    expect(notice).toBe(
      'https://example.com asked for cookie access on behalf of an embedded site, and h0x-ADE denied it.'
    )
  })
  ...
  it.each([
    ...
    ['openExternal', 'permission to open a link outside h0x-ADE'],
    ...
    ).toBe(`https://example.com asked for ${description}, and h0x-ADE denied it.`)
  })
  ...
  it('falls back to the raw permission name for anything unmapped', () => {
    ...
    ).toBe('https://example.com asked for some-future-permission, and h0x-ADE denied it.')
  })
  ...
  it('formats popup outcomes', () => {
    ...
    ).toBe('https://example.com opened a new page in h0x-ADE.')
    ...
    ).toBe('A site tried to open a popup h0x-ADE does not support here.')
  })
  ...
  it('formats certificate failures without local-server recovery advice', () => {
    ...
    expect(formatLoadFailureDescription(loadError(-202), meta)).toBe(
      "h0x-ADE doesn't trust the authority that issued the certificate for localhost:3443."
    )
    expect(formatLoadFailureDescription(loadError(-208), meta)).toBe(
      "h0x-ADE couldn't verify the certificate for localhost:3443."
    )
