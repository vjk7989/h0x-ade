...
describe('ArtifactsSettingsPane', () => {
  ...
  it('explains the complete sharing workflow', () => {
    ...
    expect(screen.getByText('Manage it in h0x-ADE')).toBeInTheDocument()
    expect(
    ...
    expect(screen.queryByText('h0x-ADE account')).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Sign in to h0x-ADE' })).not.toBeInTheDocument()
  })
  ...
  it('points web clients at the desktop app for the opt-in step', () => {
    ...
    expect(
      screen.getByText(/Open Settings → Artifacts in the h0x-ADE desktop app on the host device/)
    ).toBeInTheDocument()
  })
})
