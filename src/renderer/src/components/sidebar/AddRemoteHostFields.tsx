        ...
        {configIdentityAlias && form.identityFile.trim() === '' ? (
          <p className="text-xs text-muted-foreground">
            {translate(
              ...
              'Left empty on purpose: h0x-ADE uses every key ~/.ssh/config resolves for {{value0}}. Type a path to use just that key.',
    ...
    >
      <div className="space-y-1.5">
        <Label htmlFor="add-server-name">
          {translate('auto.components.sidebar.AddRemoteHostDialog.serverName', 'Name in h0x-ADE')}
        </Label>
        <Input
          id="add-server-name"
          value={name}
