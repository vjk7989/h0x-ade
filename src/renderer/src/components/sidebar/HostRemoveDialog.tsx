...
export function HostRemoveDialog({
  ...
  // error context owned by the h0x-ADE servers settings pane, so we deep-link
  ...
  const description =
    target.kind === 'runtime'
      ? translate(
          ...
          'This opens the h0x-ADE servers settings where you can remove this server.'
        )
      : hasWorkspaces
        ? translate(
            'auto.components.sidebar.HostRemoveDialog.hostHasWorkspacesDefault',
            'Removes {{value0}} and its credentials from this computer. Its {{value1}} stay in h0x-ADE — remote files are not touched.',
  ...
  // only forget h0x-ADE's records (offline/ghost host).
  const deleteOptionLabel = isConnected
    ...
    : translate(
        ...
        'Also remove these {{value0}} from h0x-ADE',
