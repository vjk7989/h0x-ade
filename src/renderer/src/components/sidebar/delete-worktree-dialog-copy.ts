  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from h0x-ADE. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from h0x-ADE. Git worktrees will also be removed from git and disk; folder workspaces will only remove the h0x-ADE workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from h0x-ADE. The project folder on disk will not be deleted.'
