...
function operationError(status: string): string {
  return status === 'reconnect-required'
    ? translate(
        ...
        'Reconnect your h0x-ADE account before sharing.'
      )
    : translate(
        'auto.components.skills.SkillShareDialog.unconfigured',
        'Connect an h0x-ADE Cloud account before sharing.'
      )
}
    ...
    try {
      ...
      setError(
        translate(
          ...
          'h0x-ADE could not send the cancellation request. The upload may still finish.'
        )
      )
    }
  }
