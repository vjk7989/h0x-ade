  ...
  switch (state) {
    ...
    case 'runtime-unavailable':
      return translate(
        ...
        'SSH transport is connected, but the h0x-ADE runtime is unavailable'
      )
    case 'checking':
      return translate(
        'auto.components.status.bar.RuntimeHostStatusRow.checking_host',
        'h0x-ADE is checking whether this host is reachable'
      )
    case 'reconnecting':
      return translate(
        'auto.components.status.bar.RuntimeHostStatusRow.restoring_connection',
        'h0x-ADE is trying to restore the connection'
      )
    case 'disconnected':
      return translate(
        'auto.components.status.bar.RuntimeHostStatusRow.host_unreachable',
        'h0x-ADE isn’t reachable on this host'
      )
  }
}
...
function runtimeFailureExplanation(state: RuntimeHostConnectionState): string | null {
  ...
  if (state === 'runtime-unavailable') {
    return translate(
      ...
      'The remote host may still be running; only the h0x-ADE runtime connection is unavailable.'
    )
  }
  return translate(
    'auto.components.status.bar.RuntimeHostStatusRow.contact_note',
    'The host may still be running; only the h0x-ADE connection is unavailable.'
  )
}
