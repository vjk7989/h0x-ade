...
function humanizeUnreattachableSession(error: string): string {
  const explanation = translate(
    ...
    "h0x-ADE couldn't reattach to this pane's terminal session on the host. Open a new terminal to continue."
  )
  ...
  if (humanized.includes(PANE_OWNER_UNVERIFIED_MARKER)) {
    const explanation = isPaneOwnerUnverifiedError(humanized)
      ? translate(
          ...
          "h0x-ADE couldn't safely reconnect this terminal because the host couldn't verify its saved session. h0x-ADE left the saved session unchanged. Click Retry to try reconnecting now. If it still cannot reconnect, open a new terminal."
        )
      : translate(
          'auto.components.terminal.pane.TerminalErrorToast.ownerUnknown',
          "h0x-ADE couldn't verify this terminal's owner."
        )
