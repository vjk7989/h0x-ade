...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `h0x-ADE pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior h0x-ADE session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest h0x-ADE status hints:', ...statusHints] : []),
    '',
