  ...
  switch (kind) {
    case 'invalid-input':
      return translate(
        ...
        'Enter an h0x-ADE access link or bare pairing code.'
      )
    case 'mobile-only':
      return translate(
        'auto.lib.remotePairingCopy.mobileOnly',
        'This link grants mobile-only access. Generate a link for another h0x-ADE client.'
      )
  ...
  switch (kind) {
    case 'host-identity-mismatch':
      return translate(
        ...
        'h0x-ADE reached {{endpoint}}, but that host does not match this link. Generate a new link on the other host.',
        { endpoint: endpoint ?? 'h0x-ADE' }
      )
    ...
    case 'protocol-incompatible':
      return translate(
        ...
        'Update h0x-ADE on this device and the other host, then try again.'
      )
    ...
    case 'environment-save-failed':
      return translate(
        ...
        'The host was verified, but h0x-ADE could not save it. Check the name and local settings storage, then try again.'
      )
    case 'host-unreachable':
      return translate(
        'auto.components.settings.RuntimeHostAccessForm.unavailableHelp',
        'Make sure h0x-ADE is running on the other host and that the network or SSH tunnel can reach {{endpoint}}.',
        { endpoint: endpoint ?? 'h0x-ADE' }
      )
  }
}
