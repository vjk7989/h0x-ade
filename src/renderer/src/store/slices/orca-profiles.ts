...
export const createOrcaProfilesSlice: StateCreator<AppState, [], [], OrcaProfilesSlice> = (
  ...
  fetchOrcaProfiles: async () => {
    ...
    try {
      ...
      console.error('Failed to fetch h0x-ADE profiles:', err)
  ...
  fetchOrcaProfileAuthStatus: async () => {
    try {
      ...
      console.error('Failed to fetch h0x-ADE profile auth status:', err)
      return null
    }
  },
  ...
  createLocalOrcaProfile: async (name) => {
    try {
      ...
      console.error('Failed to create h0x-ADE profile:', err)
      toast.error(
  ...
  switchOrcaProfile: async (profileId) => {
    ...
    try {
      ...
      console.error('Failed to switch h0x-ADE profile:', err)
      set({ orcaProfileSwitching: false })
      toast.error(
  ...
  transferOrcaProfileProject: async (args) => {
    try {
      ...
      console.error('Failed to transfer h0x-ADE profile project:', err)
      toast.error(
