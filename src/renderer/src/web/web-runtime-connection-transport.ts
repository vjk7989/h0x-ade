...
export class WebRuntimeConnectionTransport {
  ...
  close(options: { notifySubscriptions?: boolean } = {}): void {
    ...
    this.clearTimers()
    this.requestRegistry.rejectAll('Remote h0x-ADE runtime connection closed.')
    this.connectionWaiters.rejectAll(new Error('Remote h0x-ADE runtime connection closed.'))
  ...
  handleSocketClosed(closedWs: WebSocket): void {
    ...
    this.requestRegistry.rejectAll('Remote h0x-ADE runtime connection interrupted.')
