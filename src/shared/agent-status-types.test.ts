...
describe('parseAgentStatusPayload', () => {
  ...
  it('compacts h0x-ADE dispatch preambles so the task body survives 200-char truncation', () => {
...
Fix dispatch fallback preview for normalized status prompts`
    ...
    expect(result!.prompt.startsWith('You are working inside h0x-ADE, a multi-agent IDE.')).toBe(
      true
    )
