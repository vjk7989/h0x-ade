...
describe('pairing offer', () => {
  ...
  it('encode then decode round-trips correctly', () => {
    ...
    expect(url).toMatch(/^pavii-h0x:\/\/pair\?code=/)
  ...
  it('rejects h0x-ADE URLs outside the exact pairing route', () => {
    ...
    expect(parsePairingCode(`pavii-h0x://pairing?code=${code}`)).toBeNull()
    expect(parsePairingCode(`pavii-h0x://pair-extra?code=${code}`)).toBeNull()
    expect(() => decodePairingOffer(`pavii-h0x://pairing?code=${code}`)).toThrow(
      'Invalid pairing URL'
    )
  })

  it('rejects URLs without a pairing code', () => {
    expect(() => decodePairingOffer('pavii-h0x://pair')).toThrow('Invalid pairing URL')
  })

  it('decodes hash URLs', () => {
    const url = encodePairingOffer(offer)
    const code = new URLSearchParams(url.slice(url.indexOf('?') + 1)).get('code')!
    expect(decodePairingOffer(`pavii-h0x://pair#${code}`)).toEqual(offer)
  })
  ...
  it('rejects payloads with missing fields', () => {
    ...
    expect(() => decodePairingOffer(`pavii-h0x://pair#${base64}`)).toThrow()
  })
...
describe('parsePairingCode', () => {
  ...
  it('parses a full pavii-h0x://pair URL', () => {
    const url = encodePairingOffer(offer)
