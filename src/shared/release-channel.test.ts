...
describe('release channel', () => {
  ...
  it('keeps dev builds out of the main release repo, and apart from each other', () => {
    expect(getReleaseRepoForChannel('hourly')).toBe('vjk7989/h0x-hourly')
    expect(getReleaseRepoForChannel('daily')).toBe('vjk7989/h0x-daily')
    ...
    expect(getReleaseRepoForChannel('adhoc')).toBe('vjk7989/h0x-adhoc')
    expect(getReleaseRepoForChannel('stable')).toBe('vjk7989/h0x-ade')
    expect(getReleaseRepoForChannel('rc')).toBe('vjk7989/h0x-ade')
  })
  ...
  it('builds release-notes links against the repo that published the version', () => {
    ...
    expect(getReleaseNotesUrlForVersion('1.4.160-daily.202607281300')).toBe(
      'https://github.com/vjk7989/h0x-daily/releases/tag/v1.4.160-daily.202607281300'
    )
    expect(getReleaseNotesUrlForVersion('1.4.160')).toBe(
      'https://github.com/vjk7989/h0x-ade/releases/tag/v1.4.160'
    )
    expect(getReleaseNotesUrlForVersion('v1.4.160-rc.3')).toBe(
      'https://github.com/vjk7989/h0x-ade/releases/tag/v1.4.160-rc.3'
    )
    expect(getReleaseNotesUrlForVersion('1.4.160-adhoc.20260728140533')).toBe(
      'https://github.com/vjk7989/h0x-adhoc/releases/tag/v1.4.160-adhoc.20260728140533'
    )
    expect(getReleaseNotesUrlForVersion(null)).toBe('https://github.com/vjk7989/h0x-ade/releases')
  })
  ...
  it('sorts consecutive hourly builds newest first', () => {
    const build = (version: string): ReleaseBuild => ({
      ...
      name: null,
      publishedAt: null,
      releaseUrl: `https://github.com/vjk7989/h0x-hourly/releases/tag/v${version}`,
      installerUrl: null
    })
  ...
  it('sorts consecutive adhoc builds newest first', () => {
    const build = (version: string): ReleaseBuild => ({
      ...
      name: null,
      publishedAt: null,
      releaseUrl: `https://github.com/vjk7989/h0x-adhoc/releases/tag/v${version}`,
      installerUrl: null
    })
    const sorted = sortReleaseBuildsNewestFirst([
