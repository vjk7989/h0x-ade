...
export const SOURCE_CONTROL_ACTION_VARIABLE_INFO = {
  basePrompt: {
    description:
      'h0x-ADE’s built-in prompt for this action, including the context h0x-ADE knows how to gather safely.',
    example:
  ...
  firstPrompt: {
    description: 'The first user request that created the h0x-ADE workspace.',
    example: 'Fix CI and commit the result'
  },
  assistantMessage: {
    description: 'The initial agent response, when h0x-ADE has one available.',
