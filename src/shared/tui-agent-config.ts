...
const TUI_AGENT_CONFIG_SOURCE: Record<TuiAgent, TuiAgentConfigSource> = {
  ...
  hermes: {
    detectCmd: 'hermes',
    // Why: bare `hermes` opens the classic REPL; `--tui` starts the full-screen agent UI h0x hosts.
...
export function getTuiAgentLaunchCommand(
  ...
  // Why: remote launches must use the h0x CLI name available on that host.
