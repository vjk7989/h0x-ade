...
// it created. To avoid ever touching a user's live h0x-ADE, it REFUSES to run when
// a pre-existing app process (not a daemon) is already running that it did
...
/**
 * Find running current or legacy app processes, excluding daemons.
...
export function findAppProcesses() {
  const command = [
    ...
    `  Where-Object { $_.Name -in @('h0x-ADE.exe', 'Orca.exe') } |`,
  ...
  if (error) {
    throw new Error(`Failed to query h0x-ADE app processes: ${error.message}`)
  }
  if (code !== 0) {
    throw new Error(`Failed to query h0x-ADE app processes (exit ${code}): ${stderr || stdout}`)
  }
  ...
  try {
    ...
    throw new Error(
      `h0x-ADE app process query returned invalid JSON: ${parseError.message}\n` +
        `stdout:\n${trimmed}\nstderr:\n${stderr}`
    )
  }
}
...
export function preflight({ baselinePath, allowExistingInstall = false, installDir = null }) {
  ...
  if (isolated) {
    ...
    if (inTarget.length > 0) {
      ...
      throw new Error(
        `Refusing to run: ${inTarget.length} h0x-ADE app process(es) are running from the ` +
    ...
    if (elsewhere.length > 0) {
      warnings.push(
        `${elsewhere.length} h0x-ADE app process(es) are running from outside the isolated ` +
    ...
    throw new Error(
      `Refusing to run: ${appProcesses.length} h0x-ADE app process(es) are already ` +
  ...
  if (existingInstall && !allowExistingInstall) {
    throw new Error(
      isolated
        ...
        : `Refusing to run: an h0x-ADE install already exists at ${existingInstall}. ` +
            ...
            `(your prior build will NOT be restored), or uninstall h0x-ADE first. Clean ` +
