...
/**
 * Read-only discovery of current and legacy install registry state.
...
export function discoverInstallRegistryState() {
  const ps = [
    ...
    `  if ($dn -in @('h0x-ADE', 'Orca') -or ($il -and $il -match '\\\\Programs\\\\(h0x-ADE|orca)')) {`,
    ...
    `  if ($il -and ($sn -in @('h0x-ADE', 'Orca') -or $il -match '(h0x-ADE|orca)')) {`,
...
/** The two directories NSIS writes app shortcuts into: Start Menu + Desktop. */
...
/** Read-only: list current and legacy *.lnk shortcuts under the given dirs. */
...
function collectOrcaLnks(dir, out) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      entry.isFile() &&
      /\.lnk$/i.test(entry.name) &&
      /(?:h0x-ADE|orca)/i.test(entry.name)
    ) {
      out.push(full)
    }
  }
